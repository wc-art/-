      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-hasPermi="['system:apply:delete']"
            >删除</el-button
          >
        </template>
      </el-table-column>
...
import {
  ...
  updateApply,
  exportApply,
} from "@/api/applyParchaseUse/seed/permit";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      const seedPurchaseApplyId = row.seedPurchaseApplyId || this.ids;
      this.$confirm(
        '是否确认删除种子审核信息编号为"' + seedPurchaseApplyId + '"的数据项?',
        "警告",
        {
        ...
        .then(function () {
          return delApply(seedPurchaseApplyId);
        })
        .then(() => {
          this.getList();
    ...
    handlethrough(row) {
      ...
      console.log(data);
      updateApply(data).then((response) => {
    ...
    handleNotThrogh(row) {
      ...
      this.$confirm(
        '是否确认不通过种子审核编号为"' + seedPurchaseApplyIds + '"的数据项?',
        "警告",
        {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出所有种子审核数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
