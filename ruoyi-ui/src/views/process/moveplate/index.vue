        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['process:moveplate:permit']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
...
orderByColumn:"create_time",
  ...
  methods: {
    ...
    handlethrough(row) {
      ...
      let data = {};
      data.workMoveplateId = workMoveplateId;
      data.approval = 1;
      this.$confirm("是否确认通过该申请信息？", "提示", {
        ...
        .then(function () {
          return updateMoveplate(data).then((response) => {
    ...
    /**获取订单名称*/
    formdata_moveplateOrderList(data) {
    ...
    /**获取用户名称*/
    formdata_moveplateUserList(data) {
      let moveplate_type_map = new Map();
    ...
    getList() {
      ...
      listMoveplate(this.queryParams).then((response) => {
        ...
        // }
        console.log("this.moveplateList", this.moveplateListt);
        this.total = response.total;
        this.loading = false;
        this.formdata_moveplateOrderList(this.moveplateList);
        this.formdata_moveplateUserList(this.moveplateList);
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (var i = 0; i < this.moveplateList.length; i++) {
          queryParams.workMoveplateIds.push(
            this.moveplateList[i].workMoveplateId
          );
        }
      } else if (this.ids.length != 0) {
        queryParams.workMoveplateIds = this.ids;
      }
      this.$confirm("是否确认导出所有移盘管理数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
