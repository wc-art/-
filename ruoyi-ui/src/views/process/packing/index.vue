    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['process:packing:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['process:packing:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['process:packing:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['process:packing:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['process:packing:permit']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
...
orderByColumn:"create_time",
  ...
  methods: {
    ...
    handlethrough(row) {
      ...
      const workPackingId = row.workPackingId || this.ids;
      let data = {};
      data.workPackingId = workPackingId;
      data.approval = 1;
      this.$confirm("是否确认通过该申请信息？", "提示", {
        ...
        .then(function () {
          return updatePacking(data).then((response) => {
    ...
    /**获取订单名称*/
    formdata_packingOrderList(data) {
      let packing_type_map = new Map();
      this.order_ID.forEach((type) => {
        packing_type_map.set(type.orderId, type.orderName);
      });
      data.forEach((type) => {
        let order_name = packing_type_map.get(type.orderId);
        type.orderName = order_name;
      });
    },
    /**获取用户名称*/
    formdata_packingUserList(data) {
      let packing_type_map = new Map();
      this.user_ID.forEach((type) => {
        packing_type_map.set(type.userId, type.userName);
      });
      data.forEach((type) => {
        let user_name = packing_type_map.get(type.userId);
    ...
    getList() {
      ...
      // }
      //查询列表
      listPacking(this.queryParams).then((response) => {
        ...
        //       this.packingList.push(response.rows[j]);
        //     }
        //   }
        // } else if (this.flag == 2) {
        ...
        this.loading = false;
        this.formdata_packingOrderList(this.packingList);
        this.formdata_packingUserList(this.packingList);
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.workPackingId);
    ...
    handleUpdate(row) {
      this.reset();
      const workPackingId = row.workPackingId || this.ids;
      getPacking(workPackingId).then((response) => {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.workPackingId != null) {
            updatePacking(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addPacking(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      let that=this
      that.form2=this.form2
      const workPackingIds = row.workPackingId || this.ids;
      this.$confirm("是否确认删除该装箱管理信息?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delPacking(workPackingIds).then(response=>{
    ...
    handleExport() {
      ...
      queryParams.workPackingIds=[]
      if(this.ids.length==0){
        for(var i=0;i<this.packingList.length;i++){
          queryParams.workPackingIds.push(this.packingList[i].workPackingId)
        }
      }
      else if(this.ids.length!=0) {
        queryParams.workPackingIds = this.ids
      }
      this.$confirm("是否确认导出所有装箱管理数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportPacking(queryParams);
        })
        .then((response) => {
