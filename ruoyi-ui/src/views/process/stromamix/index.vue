    ...
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="订单编号" prop="orderId">
        <el-select v-model="queryParams.orderId" placeholder="请选择订单编号" clearable size="small">
          <el-option v-for="ID in order_ID" :key="ID.orderId" :value="ID.orderId" :label="ID.orderId"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="用户名称" prop="userId">
        <el-select v-model="queryParams.userId" placeholder="请选择用户名称" clearable size="small">
          <el-option v-for="ID in user_ID" :key="ID.userId" :value="ID.userId" :label="ID.userName"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="工作时间" prop="workTime">
        <el-date-picker clearable size="small" v-model="queryParams.workTime" type="date" value-format="yyyy-MM-dd"
          placeholder="选择工作时间">
        </el-date-picker>
      </el-form-item>
      <el-form-item label="工作地点" prop="workPlace">
        <el-select v-model="queryParams.workPlace" placeholder="请输入工作地点" clearable filterable allow-create size="small">
          <el-option v-for="ID in greenhouse_ID" :key="ID.greenhouseName" :value="ID.greenhouseName"
            :label="ID.greenhouseName"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['process:stromamix:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button v-bind:disabled="setTopBtn()||single" type="success" plain icon="el-icon-edit" size="mini"
          @click="handleUpdate" v-hasPermi="['process:stromamix:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button v-bind:disabled="setTopBtn()||multiple" type="danger" plain icon="el-icon-delete" size="mini"
          @click="handleDelete" v-hasPermi="['process:stromamix:remove']">删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" :loading="exportLoading"
          @click="handleExport" v-hasPermi="['process:stromamix:export']">导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="stromamixList" @selection-change="handleSelectionChange">
...
工作地点" align="center" prop="workPlace" />
      ...
      <el-table-column label="审批" align="center" prop="approval" width="140px">
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button v-bind:disabled="setpassBtn1(scope.row)" size="mini" type="text" icon="el-icon-check"
              @click="handlethrough(scope.row)" v-hasPermi="['process:stromamix:permit']">通过</el-button>
            <el-button v-bind:disabled="setpassBtn1(scope.row)" size="mini" type="text" icon="el-icon-close"
              @click="handleNotThrough(scope.row)" v-hasPermi="['process:stromamix:permit']">不通过</el-button>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button v-bind:disabled="setEditBtn(scope.row)" size="mini" type="text" icon="el-icon-edit"
            @click="handleUpdate(scope.row)" v-hasPermi="['process:stromamix:edit']">修改</el-button>
          <el-button v-bind:disabled="setEditBtn(scope.row)" size="mini" type="text" icon="el-icon-delete"
            @click="handleDelete(scope.row)" v-hasPermi="['process:stromamix:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="订单编号" prop="orderId">
          <el-select v-model="form.orderId" placeholder="请选择订单编号" clearable size="small">
            <el-option v-for="ID in order_ID" :key="ID.orderId" :value="ID.orderId" :label="ID.orderId"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="用户名称" prop="userId">
          <el-select v-model="form.userId" placeholder="请选择用户名称" clearable size="small">
            <el-option v-for="Id in userAdd" :key="Id.userId" :value="Id.userId" :label="Id.userAddName"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="工作时间" prop="workTime">
          <el-date-picker clearable size="small" v-model="form.workTime" type="date" value-format="yyyy-MM-dd"
            placeholder="选择工作时间">
          </el-date-picker>
        </el-form-item>
        <el-form-item label="工作地点" prop="workPlace">
          <el-select v-model="form.workPlace" placeholder="请输入工作地点" clearable filterable allow-create size="small">
            <el-option v-for="ID in greenhouse_ID" :key="ID.greenhouseName" :value="ID.greenhouseName"
              :label="ID.greenhouseName"></el-option>
          </el-select>
        </el-form-item>
...
orderByColumn:"create_time",
    ...
    methods: {
      ...
      handlethrough(row) {
        ...
        const workStromamixId = row.workStromamixId || this.ids;
        let data = {};
        data.workStromamixId = workStromamixId;
        data.approval = 1;
        this.$confirm("是否确认通过该申请信息？", "提示", {
          ...
          .then(function () {
            return updateStromamix(data).then((response) => {
      ...
      /**获取订单名称*/
      formdata_stromamixOrderList(data) {
        let stromamix_type_map = new Map();
        this.order_ID.forEach((type) => {
          stromamix_type_map.set(type.orderId, type.orderName);
        });
        data.forEach((type) => {
          let order_name = stromamix_type_map.get(type.orderId);
          type.orderName = order_name;
        });
      },
      /**获取用户名称*/
      formdata_stromamixUserList(data) {
        let stromamix_type_map = new Map();
        this.user_ID.forEach((type) => {
          stromamix_type_map.set(type.userId, type.userName);
        });
        data.forEach((type) => {
          let user_name = stromamix_type_map.get(type.userId);
      ...
      getList() {
        ...
        // }
        //查询列表
        listStromamix(this.queryParams).then((response) => {
          ...
          //       this.stromamixList.push(response.rows[j]);
          //     }
          //   }
          // } else if (this.flag == 2) {
          ...
          //   }
          // } else {
            this.stromamixList = response.rows;
          // }
          console.log("this.stromamixList", this.stromamixList);
          this.total = response.total;
          this.loading = false;
          this.formdata_stromamixOrderList(this.stromamixList);
          this.formdata_stromamixUserList(this.stromamixList);
      ...
      handleSelectionChange(selection) {
        this.ids = selection.map((item) => item.workStromamixId);
      ...
      handleUpdate(row) {
        this.reset();
        const workStromamixId = row.workStromamixId || this.ids;
        getStromamix(workStromamixId).then((response) => {
      ...
      submitForm() {
        this.$refs["form"].validate((valid) => {
          if (valid) {
            if (this.form.workStromamixId != null) {
              updateStromamix(this.form).then((response) => {
                this.msgSuccess("修改成功");
                this.open = false;
                this.getList();
              });
            } else {
              addStromamix(this.form).then((response) => {
      ...
      handleDelete(row) {
        ...
        let that = this
        that.form2 = this.form2
        const workStromamixIds = row.workStromamixId || this.ids;
        this.$confirm("是否确认删除该调配管理信息?", "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning",
        })
          .then(function () {
            return delStromamix(workStromamixIds).then(response => {
      ...
      handleExport() {
        ...
        queryParams.workStromamixIds = []
        if (this.ids.length == 0) {
          for (var i = 0; i < this.stromamixList.length; i++) {
            queryParams.workStromamixIds.push(this.stromamixList[i].workStromamixId)
          }
        }
        ...
        this.$confirm("是否确认导出调配管理数据项?", "警告", {
          ...
          .then(() => {
            ...
            return exportStromamix(queryParams);
          })
          .then((response) => {
