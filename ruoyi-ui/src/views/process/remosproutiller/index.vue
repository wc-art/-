    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['process:remosproutiller:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['process:remosproutiller:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['process:remosproutiller:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['process:remosproutiller:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="remosproutillerList" @selection-change="handleSelectionChange">
      ...
      <template slot-scope="scope">
        ...
        <el-button
          ...
          v-hasPermi="['process:remosproutiller:permit']"
        >通过</el-button>
        <el-button
          v-bind:disabled="setpassBtn1(scope.row)"
          size="mini"
...
import { listRemosproutiller, getRemosproutiller, delRemosproutiller, addRemosproutiller, updateRemosproutiller, exportRemosproutiller,listUser,listOrder,listgreenhouse } from "@/api/process/remosproutiller";
import {getUserId}from "@/api/system/user";
import{getUsergreenhouse, listUsergreenhouse}from "@/api/as/usergreenhouse";
export default {
  name: "Remosproutiller",
  components: {
  },
  data() {
    return {
  ...
  methods: {
   ...
   //控制审核后修改删除按钮
    ...
    handlethrough(row){
      ...
      const workRemosproutillerId = row.workRemosproutillerId||this.ids
      let data={}
      data.workRemosproutillerId=workRemosproutillerId;
      data.approval=1;
      this.$confirm('是否确认通过该申请信息？',"提示",{
        ...
        return updateRemosproutiller(data).then(response=>{
    ...
    /**获取订单名称*/
    formdata_remosproutillerOrderList(data){
      let remosproutiller_type_map = new Map();
      this.order_ID.forEach(type =>{
        remosproutiller_type_map.set(type.orderId,type.orderName)
      })
      data.forEach(type => {
        let order_name = remosproutiller_type_map.get(type.orderId)
        type.orderName = order_name
      });
    },
    /**获取用户名称*/
    formdata_remosproutillerUserList(data){
      let remosproutiller_type_map = new Map();
      this.user_ID.forEach(type =>{
        remosproutiller_type_map.set(type.userId,type.userName)
      })
      data.forEach(type => {
        let user_name = remosproutiller_type_map.get(type.userId)
    ...
    getList() {
      ...
      // }
//查询列表
      listRemosproutiller(this.queryParams).then(response => {
        console.log("this.flag",this.flag)
        // if(this.flag==1){
        //   console.log("我是棚员")
        //   this.remosproutillerList=[]
        //   for(var j=0;j<response.rows.length;j++){
        //     console.log("response",response)
        //     if(this.userId==response.rows[j].userId){
        //       this.remosproutillerList.push(response.rows[j]);
        //     }
        //   }
        // }
        // else if(this.flag==2){
        ...
        // else
        // {
          this.remosproutillerList = response.rows;
        // }
        console.log("this.remosproutillerList",this.remosproutillerList)
        this.total = response.total;
        this.loading = false;
        this.formdata_remosproutillerOrderList(this.remosproutillerList);
        this.formdata_remosproutillerUserList(this.remosproutillerList);
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.workRemosproutillerId)
    ...
    handleUpdate(row) {
      this.reset();
      const workRemosproutillerId = row.workRemosproutillerId || this.ids
      getRemosproutiller(workRemosproutillerId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.workRemosproutillerId != null) {
            updateRemosproutiller(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRemosproutiller(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      let that=this
      that.form2=this.form2
      const workRemosproutillerIds = row.workRemosproutillerId || this.ids;
      this.$confirm('是否确认删除该除萌蘖管理信息?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delRemosproutiller(workRemosproutillerIds).then(response=>{
    ...
    handleExport() {
      ...
      queryParams.workRemosproutillerIds=[]
      if(this.ids.length==0){
        for(var i=0;i<this.remosproutillerList.length;i++){
          queryParams.workRemosproutillerIds.push(this.remosproutillerList[i].workRemosproutillerId)
        }
      }
      else if(this.ids.length!=0) {
        queryParams.workRemosproutillerIds = this.ids
      }
      this.$confirm('是否确认导出所有除萌蘖管理数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.exportLoading = true;
          return exportRemosproutiller(queryParams);
        }).then(response => {
