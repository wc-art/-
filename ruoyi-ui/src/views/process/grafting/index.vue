    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['process:grafting:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['process:grafting:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['process:grafting:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['process:grafting:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['process:grafting:permit']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
...
orderByColumn:"create_time",
  ...
  methods: {
    ...
    handlethrough(row) {
      ...
      const workGraftingId = row.workGraftingId || this.ids;
      let data = {};
      data.workGraftingId = workGraftingId;
      data.approval = 1;
      this.$confirm("是否确认通过该申请信息？", "提示", {
        ...
        .then(function () {
          return updateGrafting(data).then((response) => {
    ...
    /**获取订单名称*/
    formdata_graftingOrderList(data) {
      let grafting_type_map = new Map();
      this.order_ID.forEach((type) => {
        grafting_type_map.set(type.orderId, type.orderName);
      });
      data.forEach((type) => {
        let order_name = grafting_type_map.get(type.orderId);
        type.orderName = order_name;
      });
    },
    /**获取用户名称*/
    formdata_graftingUserList(data) {
      let grafting_type_map = new Map();
      this.user_ID.forEach((type) => {
        grafting_type_map.set(type.userId, type.userName);
      });
      data.forEach((type) => {
        let user_name = grafting_type_map.get(type.userId);
    ...
    getList() {
      ...
      // }
      //查询列表
      listGrafting(this.queryParams).then((response) => {
        ...
        //       this.graftingList.push(response.rows[j]);
        //     }
        //   }
        // } else if (this.flag == 2) {
        ...
        //   }
        // } else {
          this.graftingList = response.rows;
        // }
        console.log("this.graftingList", this.graftingList);
        this.total = response.total;
        this.loading = false;
        this.formdata_graftingOrderList(this.graftingList);
        this.formdata_graftingUserList(this.graftingList);
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.workGraftingId);
    ...
    handleUpdate(row) {
      this.reset();
      const workGraftingId = row.workGraftingId || this.ids;
      getGrafting(workGraftingId).then((response) => {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.workGraftingId != null) {
            updateGrafting(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGrafting(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      let that=this
      that.form2=this.form2
      const workGraftingIds = row.workGraftingId || this.ids;
      this.$confirm("是否确认删除该嫁接管理信息?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delGrafting(workGraftingIds).then(response=>{
    ...
    handleExport() {
      ...
      queryParams.workGraftingIds=[]
      if(this.ids.length==0){
        for(var i=0;i<this.graftingList.length;i++){
          queryParams.workGraftingIds.push(this.graftingList[i].workGraftingId)
        }
      }
      ...
      this.$confirm("是否确认导出所有嫁接管理数据项?", "警告", {
        ...
        .then(() => {
          ...
          return exportGrafting(queryParams);
        })
        .then((response) => {
