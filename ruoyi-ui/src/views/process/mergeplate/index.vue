    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['process:mergeplate:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['process:mergeplate:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['process:mergeplate:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['process:mergeplate:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="mergeplateList" @selection-change="handleSelectionChange">
      ...
      <template slot-scope="scope">
        ...
        <el-button
          ...
          v-hasPermi="['process:mergeplate:permit']"
        >通过</el-button>
        <el-button
          v-bind:disabled="setpassBtn1(scope.row)"
          size="mini"
...
import { listMergeplate, getMergeplate, delMergeplate, addMergeplate, updateMergeplate, exportMergeplate,listUser,listOrder,listgreenhouse } from "@/api/process/mergeplate";
...
orderByColumn:"create_time",
  ...
  methods: {
    ...
    handlethrough(row){
      ...
      const workMergeplateId = row.workMergeplateId||this.ids
      let data={}
      data.workMergeplateId=workMergeplateId;
      data.approval=1;
      this.$confirm('是否确认通过该申请信息？',"提示",{
        ...
        return updateMergeplate(data).then(response=>{
    ...
    /**获取订单名称*/
    formdata_mergelpateOrderList(data){
      let mergeplate_type_map = new Map();
      this.order_ID.forEach(type =>{
        mergeplate_type_map.set(type.orderId,type.orderName)
      })
      data.forEach(type => {
        let order_name = mergeplate_type_map.get(type.orderId)
        type.orderName = order_name
      });
    },
    /**获取用户名称*/
    formdata_mergelpateUserList(data){
      let mergeplate_type_map = new Map();
      this.user_ID.forEach(type =>{
        mergeplate_type_map.set(type.userId,type.userName)
      })
      data.forEach(type => {
        let user_name = mergeplate_type_map.get(type.userId)
    ...
    getList() {
      ...
      // }
//查询列表
      listMergeplate(this.queryParams).then(response => {
        ...
        //       this.mergeplateList.push(response.rows[j]);
        //     }
        //   }
        // }
        // else if(this.flag==2){
        ...
        // else
        // {
          this.mergeplateList = response.rows;
        // }
        this.total = response.total;
        this.loading = false;
        this.formdata_mergelpateOrderList(this.mergeplateList);
        this.formdata_mergelpateUserList(this.mergeplateList);
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.workMergeplateId)
    ...
    handleUpdate(row) {
      this.reset();
      const workMergeplateId = row.workMergeplateId || this.ids
      getMergeplate(workMergeplateId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.workMergeplateId != null) {
            updateMergeplate(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMergeplate(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      let that=this
      that.form2=this.form2
      const workMergeplateIds = row.workMergeplateId || this.ids;
      this.$confirm('是否确认删除该合盘管理信息?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delMergeplate(workMergeplateIds).then(response=>{
    ...
    handleExport() {
      ...
      queryParams.workMergeplateIds=[]
      if(this.ids.length==0){
        for(var i=0;i<this.mergeplateList.length;i++){
          queryParams.workMergeplateIds.push(this.mergeplateList[i].workMergeplateId)
        }
      }
      else if(this.ids.length!=0) {
        queryParams.workMergeplateIds = this.ids
      }
      this.$confirm('是否确认导出所有合盘管理数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.exportLoading = true;
          return exportMergeplate(queryParams);
        }).then(response => {
