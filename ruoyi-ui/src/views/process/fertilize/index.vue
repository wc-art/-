    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['process:fertilize:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['process:fertilize:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['process:fertilize:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['process:fertilize:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['process:fertilize:permit']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
...
orderByColumn:"create_time",
  ...
  methods: {
    ...
    handlethrough(row) {
      ...
      const workFertilizeId = row.workFertilizeId || this.ids;
      let data = {};
      data.workFertilizeId = workFertilizeId;
      data.approval = 1;
      this.$confirm("是否确认通过该申请信息？", "提示", {
        ...
        .then(function () {
          return updateFertilize(data).then((response) => {
    ...
    /**获取订单名称*/
    formdata_fertilizeOrderList(data) {
      let fertilize_type_map = new Map();
      this.order_ID.forEach((type) => {
        fertilize_type_map.set(type.orderId, type.orderName);
      });
      data.forEach((type) => {
        let order_name = fertilize_type_map.get(type.orderId);
        type.orderName = order_name;
      });
    },
    /**获取用户名称*/
    formdata_fertilizeUserList(data) {
      let fertilize_type_map = new Map();
      this.user_ID.forEach((type) => {
        fertilize_type_map.set(type.userId, type.userName);
      });
      data.forEach((type) => {
        let user_name = fertilize_type_map.get(type.userId);
    ...
    getList() {
      ...
      // }
      //查询列表
      listFertilize(this.queryParams).then((response) => {
        ...
        //       this.fertilizeList.push(response.rows[j]);
        //     }
        //   }
        // } else if (this.flag == 2) {
        ...
        //   }
        // } else {
          this.fertilizeList = response.rows;
        // }
        console.log("this.fertilizeList", this.fertilizeList);
        this.total = response.total;
        this.loading = false;
        this.formdata_fertilizeOrderList(this.fertilizeList);
        this.formdata_fertilizeUserList(this.fertilizeList);
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.workFertilizeId);
    ...
    handleUpdate(row) {
      this.reset();
      const workFertilizeId = row.workFertilizeId || this.ids;
      getFertilize(workFertilizeId).then((response) => {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.workFertilizeId != null) {
            updateFertilize(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFertilize(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      let that=this
      that.form2=this.form2
      const workFertilizeIds = row.workFertilizeId || this.ids;
      this.$confirm("是否确认删除该施肥管理信息?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delFertilize(workFertilizeIds).then(response=>{
    ...
    handleExport() {
      ...
      queryParams.workFertilizeIds=[]
      if(this.ids.length==0){
        for(var i=0;i<this.fertilizeList.length;i++){
          queryParams.workFertilizeIds.push(this.fertilizeList[i].workFertilizeId)
        }
      }
      ...
      this.$confirm("是否确认导出所有施肥管理数据项?", "警告", {
        ...
        .then(() => {
          ...
          return exportFertilize(queryParams);
        })
        .then((response) => {
