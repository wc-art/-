    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['process:machseeding:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['process:machseeding:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['process:machseeding:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['process:machseeding:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['process:machseeding:permit']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
...
orderByColumn:"create_time",
  ...
  methods: {
    ...
    handlethrough(row) {
      ...
      const workMachseedingId = row.workMachseedingId || this.ids;
      let data = {};
      data.workMachseedingId = workMachseedingId;
      data.approval = 1;
      this.$confirm("是否确认通过该申请信息？", "提示", {
        ...
        .then(function () {
          return updateMachseeding(data).then((response) => {
    ...
    /**获取订单名称*/
    formdata_machseedingOrderList(data) {
      let machseeding_type_map = new Map();
      this.order_ID.forEach((type) => {
        machseeding_type_map.set(type.orderId, type.orderName);
      });
      data.forEach((type) => {
        let order_name = machseeding_type_map.get(type.orderId);
        type.orderName = order_name;
      });
    },
    /**获取用户名称*/
    formdata_machseedingUserList(data) {
      let machseeding_type_map = new Map();
      this.user_ID.forEach((type) => {
        machseeding_type_map.set(type.userId, type.userName);
      });
      data.forEach((type) => {
        let user_name = machseeding_type_map.get(type.userId);
    ...
    getList() {
      ...
      // }
      //查询列表
      listMachseeding(this.queryParams).then((response) => {
        console.log("this.flag", this.flag);
        // 前端数据筛选
        // if (this.flag == 1) {
        //   console.log("我是棚员");
        //   this.machseedingList = [];
        //   for (var j = 0; j < response.rows.length; j++) {
        //     console.log("response", response);
        //     if (this.userId == response.rows[j].userId) {
        //       this.machseedingList.push(response.rows[j]);
        //     }
        //   }
        // } else if (this.flag == 2) {
        ...
        //   }
        // } else {
          this.machseedingList = response.rows;
        // }
        console.log("this.machseedingList", this.machseedingList);
        this.total = response.total;
        this.loading = false;
        this.formdata_machseedingOrderList(this.machseedingList);
        this.formdata_machseedingUserList(this.machseedingList);
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.workMachseedingId);
    ...
    handleUpdate(row) {
      this.reset();
      const workMachseedingId = row.workMachseedingId || this.ids;
      getMachseeding(workMachseedingId).then((response) => {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.workMachseedingId != null) {
            updateMachseeding(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addMachseeding(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      let that=this
      that.form2=this.form2
      const workMachseedingIds = row.workMachseedingId || this.ids;
      this.$confirm("是否确认删除该机器播种管理信息?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delMachseeding(workMachseedingIds).then(response=>{
    ...
    handleExport() {
      ...
      queryParams.workMachseedingIds=[]
      if(this.ids.length==0){
        for(var i=0;i<this.machseedingList.length;i++){
          queryParams.workMachseedingIds.push(this.machseedingList[i].workMachseedingId)
        }
      }
      else if(this.ids.length!=0) {
        queryParams.workMachseedingIds = this.ids
      }
      this.$confirm("是否确认导出所有机器播种管理数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportMachseeding(queryParams);
        })
        .then((response) => {
