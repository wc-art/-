    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['process:artiseeding:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['process:artiseeding:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['process:artiseeding:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['process:artiseeding:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['process:artiseeding:permit']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
...
export default {
  ...
  methods: {
    ...
    handlethrough(row) {
      ...
      const workArtiseedingId = row.workArtiseedingId || this.ids;
      let data = {};
      data.workArtiseedingId = workArtiseedingId;
      data.approval = 1;
      this.$confirm("是否确认通过该申请信息？", "提示", {
        ...
        .then(function () {
          return updateArtiseeding(data).then((response) => {
    ...
    /**获取订单名称*/
    formdata_artiseedingOrderList(data) {
      let artiseeding_type_map = new Map();
      this.order_ID.forEach((type) => {
        artiseeding_type_map.set(type.orderId, type.orderName);
      });
      data.forEach((type) => {
        let order_name = artiseeding_type_map.get(type.orderId);
        type.orderName = order_name;
      });
    },
    /**获取用户名称*/
    formdata_artiseedingUserList(data) {
      let artiseeding_type_map = new Map();
      this.user_ID.forEach((type) => {
        artiseeding_type_map.set(type.userId, type.userName);
      });
      data.forEach((type) => {
        let user_name = artiseeding_type_map.get(type.userId);
    ...
    getList() {
      ...
      // console.log("2关联的大棚this.greenhouseIds", this.greenhouseIds);
      ...
      // }
      //查询列表
      listArtiseeding(this.queryParams).then((response) => {
        // 前端数据筛选
        // console.log("this.flag", this.flag);
        // if (this.flag == 1) {
        //   console.log("我是棚员");
        //   this.artiseedingList = [];
        //   for (var j = 0; j < response.rows.length; j++) {
        //     console.log("response", response);
        //     if (this.userId == response.rows[j].userId) {
        //       this.artiseedingList.push(response.rows[j]);
        //     }
        //   }
        // } else if (this.flag == 2) {
        ...
        //   }
        // } else {
        this.artiseedingList = response.rows;
        // }
        console.log("this.artiseedingList", this.artiseedingList);
        this.total = response.total;
        this.loading = false;
        this.formdata_artiseedingOrderList(this.artiseedingList);
        this.formdata_artiseedingUserList(this.artiseedingList);
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.workArtiseedingId);
    ...
    handleUpdate(row) {
      this.reset();
      const workArtiseedingId = row.workArtiseedingId || this.ids;
      getArtiseeding(workArtiseedingId).then((response) => {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.workArtiseedingId != null) {
            updateArtiseeding(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addArtiseeding(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      let that = this;
      that.form2 = this.form2;
      const workArtiseedingIds = row.workArtiseedingId || this.ids;
      this.$confirm("是否确认删除该人工播种信息?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delArtiseeding(workArtiseedingIds).then((response) => {
    ...
    handleExport() {
      ...
      queryParams.workArtiseedingIds = [];
      if (this.ids.length == 0) {
        for (var i = 0; i < this.artiseedingList.length; i++) {
          queryParams.workArtiseedingIds.push(
            this.artiseedingList[i].workArtiseedingId
          );
        }
      } else if (this.ids.length != 0) {
        queryParams.workArtiseedingId = this.ids;
      }
      this.$confirm("是否确认导出所有人工播种数据项?", "警告", {
        ...
        .then(() => {
          ...
          return exportArtiseeding(queryParams);
        })
        .then((response) => {
