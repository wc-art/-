    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['process:takeoffhat:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['process:takeoffhat:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['process:takeoffhat:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['process:takeoffhat:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="takeoffhatList" @selection-change="handleSelectionChange">
      ...
      <template slot-scope="scope">
        ...
        <el-button
          ...
          v-hasPermi="['process:takeoffhat:permit']"
        >通过</el-button>
        <el-button
          v-bind:disabled="setpassBtn1(scope.row)"
          size="mini"
...
import { listTakeoffhat, getTakeoffhat, delTakeoffhat, addTakeoffhat, updateTakeoffhat, exportTakeoffhat,listUser,listOrder,listgreenhouse } from "@/api/process/takeoffhat";
   ...
   /**通过按钮操作*/
    handlethrough(row){
     ...
     that.form2=this.form2
      ...
      const workTakeoffhatId = row.workTakeoffhatId||this.ids
      let data={}
      data.workTakeoffhatId=workTakeoffhatId;
      data.approval=1;
      this.$confirm('是否确认通过该申请信息？',"提示",{
        ...
        return updateTakeoffhat(data).then(response=>{
    ...
    handleNotThrough(row){
      ...
      this.$confirm('是否确认不通过该申请信息？',"警告",{
        ...
        return updateTakeoffhatdata.then(response=>{
          console.log(response)
    ...
    /**获取订单名称*/
    formdata_takeoffhatOrderList(data){
      let takeoffhat_type_map = new Map();
      this.order_ID.forEach(type =>{
        takeoffhat_type_map.set(type.orderId,type.orderName)
      })
      data.forEach(type => {
        let order_name = takeoffhat_type_map.get(type.orderId)
        console.log("orderName",order_name)
        type.orderName = order_name
        console.log(type.orderName)
      });
    },
    /**获取用户名称*/
    formdata_takeoffhatUserList(data){
      let takeoffhat_type_map = new Map();
      this.user_ID.forEach(type =>{
        takeoffhat_type_map.set(type.userId,type.userName)
      })
      data.forEach(type => {
        let user_name = takeoffhat_type_map.get(type.userId)
    ...
    getList() {
      ...
      // }
//查询列表
      listTakeoffhat(this.queryParams).then(response => {
        ...
        //       this.takeoffhatList.push(response.rows[j]);
        //     }
        //   }
        // }
        // else if(this.flag==2){
        ...
        // else
        // {
          this.takeoffhatList = response.rows;
        // }
        console.log("this.takeoffhatList",this.takeoffhatList)
        this.total = response.total;
        this.loading = false;
        this.formdata_takeoffhatOrderList(this.takeoffhatList);
        this.formdata_takeoffhatUserList(this.takeoffhatList);
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.workTakeoffhatId)
    ...
    handleUpdate(row) {
      this.reset();
      const workTakeoffhatId = row.workTakeoffhatId || this.ids
      getTakeoffhat(workTakeoffhatId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.workTakeoffhatId != null) {
            updateTakeoffhat(this.form).then(response => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addTakeoffhat(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      let that=this
      that.form2=this.form2
      const workTakeoffhatIds = row.workTakeoffhatId || this.ids;
      this.$confirm('是否确认删除该脱帽管理信息?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delTakeoffhat(workTakeoffhatIds).then(response=>{
    ...
    handleExport() {
      ...
      queryParams.workTakeoffhatId=[]
      if(this.ids.length==0){
        for(var i=0;i<this.takeoffhatList.length;i++){
          queryParams.workTakeoffhatIds.push(this.takeoffhatList[i].workTakeoffhatId)
        }
      }
      else if(this.ids.length!=0) {
        queryParams.workTakeoffhatIds = this.ids
      }
      this.$confirm('是否确认导出所有脱帽管理数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.exportLoading = true;
          return exportTakeoffhat(queryParams);
        }).then(response => {
