<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['expertManage:PestQuality:add']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['expertManage:PestQuality:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" plain icon="el-icon-delete" size="mini" :disabled="multiple" @click="handleDelete"
          v-hasPermi="['expertManage:PestQuality:remove']">删除</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-document" @click="handleDetailed(scope.row)"
            v-hasPermi="['expertManage:PestQuality:detailed']">详情</el-button>
    ...
    <add-or-update :title="title" :open="open" :form="form" :rules="rules" @reset="reset" @cancel="cancel"
      @getList="getList" categoryName="病害虫数据库" from="expert_pest_control"></add-or-update>

    <detailed title="病害虫数据库详情" :openDetailed="openDetailed" :form="form" @cancelDetailed="cancelDetailed"
      categoryName="病害虫数据库" from="expert_pest_control"></detailed>
  </div>
</template>

<script>
  import { listPest, getPest, delPest } from "@/api/expertManage/pest";
...
orderByColumn:"create_time",
    ...
    methods: {
      ...
      getList() {
        this.loading = true;
        listPest(this.queryParams).then(response => {
      ...
      handleUpdate(row) {
        ...
        getPest(expertIds).then(response => {
      ...
      handleDelete(row) {
        ...
        this.$confirm('是否确认删除病害虫数据库序号为"' + xh + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function () {
          return delPest(expertIds);
        }).then(() => {
          this.getList();
      ...
      handleDetailed(row) {
        this.reset()
        const expertId = row.expertdbId
        getPest(expertId).then(response => {
          this.form = response.data;
