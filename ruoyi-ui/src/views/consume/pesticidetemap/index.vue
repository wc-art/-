<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['consume:pesticidetemap:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['consume:pesticidetemap:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consume:pesticidetemap:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['consume:pesticidetemap:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:pesticidetemcon:add']"
            >申请</el-button
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:pesticidetemap:back']"
            >退回</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="编号" prop="pesticideApplyTemporaryId">
          <el-input v-model="form.pesticideApplyTemporaryId" disabled />
        </el-form-item>
        <el-form-item label="农药类型" prop="pesticideTypeId">
          <el-input v-model="form.pesticideType" disabled />
        </el-form-item>
        <el-form-item label="农药名称" prop="pesticideNameId">
          <el-input v-model="form.pesticideName" disabled />
        </el-form-item>
...
import {
  ...
  delPesticidetemap,
  addPesticidetemap,
  updatePesticidetemap,
  exportPesticidetemap,
} from "@/api/consume/pesticidetemap";
...
export default {
  ...
  watch: {
    "$route.path": function (newVal, oldVal) {
      if (newVal === "/production/consumetem/consumetem4/pesticidetemap") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listPesticidetemap(this.queryParams).then((response) => {
        this.pesticidetemapList = response.rows;
    ...
    handleApply(row) {
      ...
      getPesticidetemap(pesticideApplyTemporaryId).then((response) => {
        ...
        if (valid) {
          if (this.form.pesticideApplyTemporaryId != null) {
            ...
            upAmount(this.form)
              ...
              .then((res) => {
                if (code == 200) {
                  upCumStatus(this.form.pesticideApplyTemporaryId).then(
                    (response) => {}
                  );
                  addPesticidecon(this.form).then((response) => {
    ...
    handleBack(row) {
      const pesticideApplyTemporaryId =
        row.pesticideApplyTemporaryId || this.ids;
      this.$confirm("是否确认退回农药临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return back(pesticideApplyTemporaryId);
        })
        .then(() => {
          this.getList();
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.pesticideApplyTemporaryId != null) {
            updatePesticidetemap(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addPesticidetemap(this.form).then((response) => {
    ...
    handleDelete(row) {
      const pesticideApplyTemporaryIds =
        row.pesticideApplyTemporaryId || this.ids;
      this.$confirm("是否确认删除农药临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delPesticidetemap(pesticideApplyTemporaryIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.pesticidetemapList.length; i++) {
          queryParams.exportIds.push(this.pesticidetemapList[i].pesticideApplyTemporaryId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有农药临时领用数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportPesticidetemap(queryParams);
        })
        .then((response) => {
