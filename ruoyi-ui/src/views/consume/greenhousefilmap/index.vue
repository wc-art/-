<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['consume:greenhousefilmap:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['consume:greenhousefilmap:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consume:greenhousefilmap:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['consume:greenhousefilmap:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:greenhousefilmcon:add']"
            >申请</el-button
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:greenhousefilmap:back']"
            >退回</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="编号" prop="greenhousefilmApplyId">
          <el-input v-model="form.greenhousefilmApplyId" disabled />
        </el-form-item>
...
import {
  ...
  getUserName,
  listGreenhousefilmap,
  getGreenhousefilmap,
  delGreenhousefilmap,
  addGreenhousefilmap,
  updateGreenhousefilmap,
  exportGreenhousefilmap,
} from "@/api/consume/greenhousefilmap";
import { addFertilizercon } from "@/api/consume/fertilizerap";

export default {
  name: "Greenhousefilmap",
  components: {},
  data() {
    return {
      // 遮罩层
  ...
  watch: {
    "$route.path": function (newVal, oldVal) {
      if (newVal === "/production/consume/consume10/greenhousefilmap") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listGreenhousefilmap(this.queryParams).then((response) => {
        this.greenhousefilmapList = response.rows;
    ...
    handleApply(row) {
      ...
      getGreenhousefilmap(greenhousefilmApplyId).then((response) => {
        ...
        if (valid) {
          if (this.form.greenhousefilmApplyId != null) {
            ...
            upAmount(this.form)
              ...
              .then((res) => {
                if (code == 200) {
                  upCumStatus(this.form.greenhousefilmApplyId).then(
                    (response) => {}
                  );
    ...
    handleBack(row) {
      ...
      this.$confirm("是否确认退回棚膜领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return back(greenhousefilmApplyIds);
        })
        .then(() => {
          this.getList();
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.greenhousefilmApplyId != null) {
            updateGreenhousefilmap(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGreenhousefilmap(this.form).then((response) => {
    ...
    handleDelete(row) {
      const greenhousefilmApplyIds = row.greenhousefilmApplyId || this.ids;
      this.$confirm("是否确认删除棚膜领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delGreenhousefilmap(greenhousefilmApplyIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.greenhousefilmapList.length; i++) {
          queryParams.exportIds.push(this.greenhousefilmapList[i].greenhousefilmApplyId);
        }
      ...
      this.$confirm("是否确认导出所有棚膜领用数据项?", "警告", {
        ...
        .then(() => {
          ...
          return exportGreenhousefilmap(queryParams);
        })
        .then((response) => {
