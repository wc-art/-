<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['consume:fertilizerap:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['consume:fertilizerap:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consume:fertilizerap:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['consume:fertilizerap:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    <el-table
      v-loading="loading"
      :data="fertilizerapList"
      @selection-change="handleSelectionChange"
    >
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="编号" align="center" prop="fertilizerApplyId" />
      <el-table-column
        label="订单分配号"
        align="center"
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:fertilizercon:add']"
            >申请</el-button
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:fertilizerap:back']"
            >退回</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="编号" prop="fertilizerApplyId">
          <el-input v-model="form.fertilizerApplyId" disabled />
        </el-form-item>
...
export default {
  ...
  watch: {
    "$route.path": function (newVal, oldVal) {
      if (newVal === "/production/consume/consume6/fertilizerap") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFertilizerap(this.queryParams).then((response) => {
        this.fertilizerapList = response.rows;
    ...
    handleApply(row) {
      ...
      getFertilizerap(fertilizerApplyId).then((response) => {
        ...
        if (valid) {
          if (this.form.fertilizerApplyId != null) {
            ...
            upAmount(this.form)
              ...
              .then((res) => {
                if (code == 200) {
                  upCumStatus(this.form.fertilizerApplyId).then(
                    (response) => {}
                  );
    ...
    handleBack(row) {
      ...
      this.$confirm("是否确认退回化肥领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return back(fertilizerApplyIds);
        })
        .then(() => {
          this.getList();
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.fertilizerApplyId != null) {
            updateFertilizerap(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFertilizerap(this.form).then((response) => {
    ...
    handleDelete(row) {
      const fertilizerApplyIds = row.fertilizerApplyId || this.ids;
      this.$confirm("是否确认删除化肥领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delFertilizerap(fertilizerApplyIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.fertilizerapList.length; i++) {
          queryParams.exportIds.push(this.fertilizerapList[i].fertilizerApplyId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有化肥领用数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportFertilizerap(queryParams);
        })
        .then((response) => {
