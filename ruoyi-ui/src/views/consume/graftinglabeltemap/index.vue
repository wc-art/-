<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['consume:graftinglabeltemap:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['consume:graftinglabeltemap:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consume:graftinglabeltemap:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['consume:graftinglabeltemap:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    >
      ...
      <el-table-column
        label="申请编号"
        align="center"
        prop="graftinglabelApplyTemporaryId"
      />
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:graftinglabeltemcon:add']"
            >申请</el-button
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:graftinglabeltemap:back']"
            >退回</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="编号" prop="graftinglabelApplyTemporaryId">
          <el-input v-model="form.graftinglabelApplyTemporaryId" disabled />
        </el-form-item>
...
import {
  ...
  addGraftinglabelcon,
  getUserName,
  listGraftinglabeltemap,
  getGraftinglabeltemap,
  delGraftinglabeltemap,
  addGraftinglabeltemap,
  updateGraftinglabeltemap,
  exportGraftinglabeltemap,
} from "@/api/consume/graftinglabeltemap";
import { addFertilizercon } from "@/api/consume/fertilizerap";

export default {
  name: "Graftinglabeltemap",
  components: {},
  data() {
    return {
      // 遮罩层
  ...
  watch: {
    "$route.path": function (newVal, oldVal) {
      if (newVal === "/production/consumetem/consumetem8/graftinglabeltemap") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listGraftinglabeltemap(this.queryParams).then((response) => {
        this.graftinglabeltemapList = response.rows;
    ...
    reset() {
      this.form = {
        graftinglabelApplyTemporaryId: null,
        graftinglabelTypeId: null,
        graftinglabelType: null,
        graftinglabelSpecification: null,
        originCompany: null,
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.graftinglabelApplyTemporaryId);
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加嫁接签临时领用";
    },
    /** 领用按钮操作 */
    handleApply(row) {
      this.reset();
      const graftinglabelApplyTemporaryId =
        row.graftinglabelApplyTemporaryId || this.ids;
      getGraftinglabeltemap(graftinglabelApplyTemporaryId).then((response) => {
    ...
    submitOpera() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.graftinglabelApplyTemporaryId != null) {
            var code = 200;
            upAmount(this.form)
              .then((response) => {
                this.open = false;
              ...
              .then((res) => {
                if (code == 200) {
                  upCumStatus(this.form.graftinglabelApplyTemporaryId).then(
                    (response) => {}
                  );
    ...
    handleBack(row) {
      const graftinglabelApplyTemporaryId =
        row.graftinglabelApplyTemporaryId || this.ids;
      this.$confirm("是否确认退回嫁接签临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return back(graftinglabelApplyTemporaryId);
        })
        .then(() => {
          this.getList();
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.graftinglabelApplyTemporaryId != null) {
            updateGraftinglabeltemap(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGraftinglabeltemap(this.form).then((response) => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const graftinglabelApplyTemporaryIds =
        row.graftinglabelApplyTemporaryId || this.ids;
      this.$confirm("是否确认删除嫁接签临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delGraftinglabeltemap(graftinglabelApplyTemporaryIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.graftinglabeltemapList.length; i++) {
          queryParams.exportIds.push(this.graftinglabeltemapList[i].graftinglabelApplyTemporaryId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有嫁接签临时领用数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportGraftinglabeltemap(queryParams);
        })
        .then((response) => {
