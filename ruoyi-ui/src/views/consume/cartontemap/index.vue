<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['consume:cartontemap:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['consume:cartontemap:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consume:cartontemap:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['consume:cartontemap:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:cartontemcon:add']"
            >申请</el-button
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:cartontemap:back']"
            >退回</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="编号" prop="cartonApplyTemporaryId">
          <el-input v-model="form.cartonApplyTemporaryId" disabled />
        </el-form-item>
        <el-form-item label="类型" prop="cartonTypeId">
          <el-input v-model="form.cartonType" disabled />
        </el-form-item>
        <el-form-item label="规格" prop="cartonSpecification">
          <el-input v-model="form.cartonSpecification" disabled />
        </el-form-item>
        <el-form-item label="购自公司" prop="originCompany">
...
export default {
  ...
  watch: {
    "$route.path": function (newVal, oldVal) {
      if (newVal === "/production/consumetem/consumetem11/cartontemap") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCartontemap(this.queryParams).then((response) => {
        this.cartontemapList = response.rows;
    ...
    handleApply(row) {
      ...
      getCartontemap(cartonApplyTemporaryId).then((response) => {
        ...
        if (valid) {
          if (this.form.cartonApplyTemporaryId != null) {
            ...
            upAmount(this.form)
              ...
              .then((res) => {
                if (code == 200) {
                  upCumStatus(this.form.cartonApplyTemporaryId).then(
                    (response) => {}
                  );
                  addCartoncon(this.form).then((response) => {
    ...
    handleBack(row) {
      ...
      this.$confirm("是否确认退回纸箱临时领用?", "警告", {
        ...
        .then(function () {
          return back(cartonApplyTemporaryId);
        })
        .then(() => {
          this.getList();
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.cartonApplyTemporaryId != null) {
            updateCartontemap(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCartontemap(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$confirm("是否确认删除纸箱临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delCartontemap(cartonApplyTemporaryIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.cartontemapList.length; i++) {
          queryParams.exportIds.push(this.cartontemapList[i].cartonApplyTemporaryId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有纸箱临时领用数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportCartontemap(queryParams);
        })
        .then((response) => {
