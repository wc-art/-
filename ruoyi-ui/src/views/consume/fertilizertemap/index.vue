<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['consume:fertilizertemap:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['consume:fertilizertemap:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consume:fertilizertemap:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['consume:fertilizertemap:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    >
      ...
      <el-table-column
        label="申请编号"
        align="center"
        prop="fertilizerApplyTemporaryId"
      />
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:fertilizertemcon:add']"
            >申请</el-button
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:fertilizertemap:back']"
            >退回</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="编号" prop="fertilizerApplyTemporaryId">
          <el-input v-model="form.fertilizerApplyTemporaryId" disabled />
        </el-form-item>
        <el-form-item label="肥料类型" prop="fertilizerTypeId">
          <el-input v-model="form.fertilizerType" disabled />
        </el-form-item>
        <el-form-item label="肥料名称" prop="fertilizerNameId">
          <el-input v-model="form.fertilizerName" disabled />
        </el-form-item>
        <el-form-item label="NPK含量" prop="npkContent">
          <el-input v-model="form.npkContent" disabled />
        </el-form-item>
        <el-form-item label="肥料品牌" prop="brand">
...
import {
  ...
  delFertilizertemap,
  addFertilizertemap,
  updateFertilizertemap,
  exportFertilizertemap,
} from "@/api/consume/fertilizertemap";
import Editor from "@/components/Editor";

export default {
  name: "Fertilizertemap",
  components: {
    Editor,
  },
  data() {
  ...
  watch: {
    "$route.path": function (newVal, oldVal) {
      if (newVal === "/production/consumetem/consumetem6/fertilizertemap") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFertilizertemap(this.queryParams).then((response) => {
        this.fertilizertemapList = response.rows;
    ...
    reset() {
      this.form = {
        fertilizerApplyTemporaryId: null,
        fertilizerTypeId: null,
        fertilizerNameId: null,
        npkContent: null,
        brand: null,
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.fertilizerApplyTemporaryId);
    ...
    handleApply(row) {
      this.reset();
      const fertilizerApplyTemporaryId =
        row.fertilizerApplyTemporaryId || this.ids;
      getFertilizertemap(fertilizerApplyTemporaryId).then((response) => {
    ...
    submitOpera() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.fertilizerApplyTemporaryId != null) {
            var code = 200;
            upAmount(this.form)
              .then((response) => {
                this.open = false;
              ...
              .then((res) => {
                if (code == 200) {
                  upCumStatus(this.form.fertilizerApplyTemporaryId).then(
                    (response) => {}
                  );
                  addFertilizercon(this.form).then((response) => {
    ...
    handleBack(row) {
      const fertilizerApplyTemporaryId =
        row.fertilizerApplyTemporaryId || this.ids;
      this.$confirm("是否确认退回化肥临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return back(fertilizerApplyTemporaryId);
        })
        .then(() => {
          this.getList();
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.fertilizerApplyTemporaryId != null) {
            updateFertilizertemap(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFertilizertemap(this.form).then((response) => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const fertilizerApplyTemporaryIds =
        row.fertilizerApplyTemporaryId || this.ids;
      this.$confirm("是否确认删除化肥临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delFertilizertemap(fertilizerApplyTemporaryIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.fertilizertemapList.length; i++) {
          queryParams.exportIds.push(this.fertilizertemapList[i].fertilizerApplyTemporaryId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有化肥临时领用数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportFertilizertemap(queryParams);
        })
        .then((response) => {
