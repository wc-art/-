<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="基质类型" prop="stromaType">
        <el-input
          v-model="queryParams.stromaType"
          placeholder="请输入基质类型"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['consume:stromatemap:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['consume:stromatemap:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consume:stromatemap:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['consume:stromatemap:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    >
      ...
      <el-table-column
        label="申请编号"
        align="center"
        prop="stromaApplyTemporayId"
      />
      <el-table-column label="基质种类" align="center" prop="stromaKind" />
      <el-table-column label="基质类型" align="center" prop="stromaType" />
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:stromatemcon:add']"
            >申请</el-button
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:stromatemap:back']"
            >退回</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="编号" prop="stromaApplyTemporayId">
          <el-input v-model="form.stromaApplyTemporayId" disabled />
        </el-form-item>
        <el-form-item label="基质种类" prop="stromaKindId">
          <el-input v-model="form.stromaKind" disabled />
        </el-form-item>
        <el-form-item label="基质类型" prop="stromaTypeId">
          <el-input v-model="form.stromaType" disabled />
        </el-form-item>
        <el-form-item label="基质品牌" prop="brand">
...
export default {
  ...
  watch: {
    "$route.path": function (newVal, oldVal) {
      if (newVal === "/production/consumetem/consumetem2/stromatemap") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listStromatemap(this.queryParams).then((response) => {
        this.stromatemapList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.stromaApplyTemporayId);
    ...
    handleApply(row) {
      this.reset();
      const stromaApplyTemporayId = row.stromaApplyTemporayId || this.ids;
      getStromatemap(stromaApplyTemporayId).then((response) => {
    ...
    submitOpera() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.stromaApplyTemporayId != null) {
            var code = 200;
            upAmount(this.form)
              .then((response) => {
                this.open = false;
              ...
              .then((res) => {
                if (code == 200) {
                  upCumStatus(this.form.stromaApplyTemporayId).then(
                    (response) => {}
                  );
                  addStromacon(this.form).then((response) => {
    ...
    handleBack(row) {
      const stromaApplyTemporayId = row.stromaApplyTemporayId || this.ids;
      this.$confirm("是否确认退回基质临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return back(stromaApplyTemporayId);
        })
        .then(() => {
          this.getList();
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.stromaApplyTemporayId != null) {
            updateStromatemap(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addStromatemap(this.form).then((response) => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const stromaApplyTemporayIds = row.stromaApplyTemporayId || this.ids;
      this.$confirm("是否确认删除基质临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delStromatemap(stromaApplyTemporayIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.stromatemapList.length; i++) {
          queryParams.exportIds.push(this.stromatemapList[i].stromaApplyTemporayId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有基质临时领用数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportStromatemap(queryParams);
        })
        .then((response) => {
