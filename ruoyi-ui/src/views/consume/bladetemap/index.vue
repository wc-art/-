<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['consume:bladetemap:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['consume:bladetemap:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consume:bladetemap:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['consume:bladetemap:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:bladetemcon:add']"
            >申请</el-button
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:bladetemap:back']"
            >退回</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="编号" prop="bladeApplyTemporaryId">
          <el-input v-model="form.bladeApplyTemporaryId" disabled />
        </el-form-item>
        <el-form-item label="类型" prop="bladeTypeId">
          <el-input v-model="form.bladeType" disabled />
        </el-form-item>
        <el-form-item label="品牌" prop="brand">
...
export default {
  ...
  watch: {
    "$route.path": function (newVal, oldVal) {
      if (newVal === "/production/consumetem/consumetem9/bladetemap") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listBladetemap(this.queryParams).then((response) => {
        this.bladetemapList = response.rows;
    ...
    handleApply(row) {
      ...
      getBladetemap(bladeApplyTemporaryId).then((response) => {
        ...
        if (valid) {
          if (this.form.bladeApplyTemporaryId != null) {
            ...
            upAmount(this.form)
              ...
              .then((res) => {
                if (code == 200) {
                  upCumStatus(this.form.bladeApplyTemporaryId).then(
                    (response) => {}
                  );
                  addBladecon(this.form).then((response) => {
    ...
    handleBack(row) {
      const bladeApplyTemporaryId = row.bladeApplyTemporaryId || this.ids;
      this.$confirm("是否确认退回纸箱临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return back(bladeApplyTemporaryId);
        })
        .then(() => {
          this.getList();
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.bladeApplyTemporaryId != null) {
            updateBladetemap(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addBladetemap(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$confirm("是否确认删除刀片临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delBladetemap(bladeApplyTemporaryIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.bladetemapList.length; i++) {
          queryParams.exportIds.push(this.bladetemapList[i].bladeApplyTemporaryId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有刀片临时领用数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportBladetemap(queryParams);
        })
        .then((response) => {
