<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['consume:regulatortemap:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['consume:regulatortemap:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consume:regulatortemap:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['consume:regulatortemap:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:regulatortemcon:add']"
            >申请</el-button
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:regulatortemap:back']"
            >退回</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="编号" prop="regulatorApplyTemporaryId">
          <el-input v-model="form.regulatorApplyTemporaryId" disabled />
        </el-form-item>
...
import {
  ...
  delRegulatortemap,
  addRegulatortemap,
  updateRegulatortemap,
  exportRegulatortemap,
} from "@/api/consume/regulatortemap";
...
export default {
  ...
  watch: {
    "$route.path": function (newVal, oldVal) {
      if (newVal === "/production/consumetem/consumetem5/regulatortemap") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listRegulatortemap(this.queryParams).then((response) => {
        this.regulatortemapList = response.rows;
    ...
    handleApply(row) {
      ...
      getRegulatortemap(regulatorApplyTemporaryId).then((response) => {
        ...
        if (valid) {
          if (this.form.regulatorApplyTemporaryId != null) {
            ...
            upAmount(this.form)
              ...
              .then((res) => {
                if (code == 200) {
                  upCumStatus(this.form.regulatorApplyTemporaryId).then(
                    (response) => {}
                  );
    ...
    handleBack(row) {
      const regulatorApplyTemporaryId =
        row.regulatorApplyTemporaryId || this.ids;
      this.$confirm("是否确认退回调节剂临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return back(regulatorApplyTemporaryId);
        })
        .then(() => {
          this.getList();
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.regulatorApplyTemporaryId != null) {
            updateRegulatortemap(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRegulatortemap(this.form).then((response) => {
    ...
    handleDelete(row) {
      const regulatorApplyTemporaryIds =
        row.regulatorApplyTemporaryId || this.ids;
      this.$confirm("是否确认删除调节剂临时领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delRegulatortemap(regulatorApplyTemporaryIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.regulatortemapList.length; i++) {
          queryParams.exportIds.push(this.regulatortemapList[i].regulatorApplyTemporaryId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有调节剂临时领用数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportRegulatortemap(queryParams);
        })
        .then((response) => {
