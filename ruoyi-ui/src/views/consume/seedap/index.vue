<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['consume:seedap:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consume:seedap:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['consume:seedap:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:seedcon:add']"
            >申请</el-button
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:seedap:back']"
            >退回</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="编号" prop="seedApplyId">
          <el-input v-model="form.seedApplyId" disabled />
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        orderAllocationId: [
          { required: true, message: "订单分配不能为空", trigger: "blur" },
        ],
        applyAmount: [
  ...
  watch: {
    "$route.path": function (newVal, oldVal) {
      if (newVal === "/production/consume/consume1/seedap") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listSeedap(this.queryParams).then((response) => {
        this.seedapList = response.rows;
        this.total = response.total;
        this.loading = false;
        this.formdata_vegertable(this.seedapList);
        this.formdata_userList(this.seedapList);
      });
    },
    // 取消按钮
    cancel() {
    ...
    handleApply(row) {
      ...
      getSeedap(seedApplyId).then((response) => {
        ...
        if (valid) {
          if (this.form.seedApplyId != null) {
            ...
            upAmount(this.form)
              ...
              .then((res) => {
                if (code == 200) {
                  upCumStatus(this.form.seedApplyId).then((response) => {});
    ...
    handleDelete(row) {
      const seedApplyIds = row.seedApplyId || this.ids;
      this.$confirm("是否确认删除种子领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delSeedap(seedApplyIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.seedapList.length; i++) {
          queryParams.exportIds.push(this.seedapList[i].seedApplyId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有种子领用数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportSeedap(queryParams);
        })
        .then((response) => {
