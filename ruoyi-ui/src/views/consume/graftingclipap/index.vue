<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['consume:graftingclipap:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['consume:graftingclipap:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['consume:graftingclipap:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['consume:graftingclipap:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:graftingclipcon:add']"
            >申请</el-button
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['consume:graftingclipap:back']"
            >退回</el-button
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="编号" prop="graftingclipApplyId">
          <el-input v-model="form.graftingclipApplyId" disabled />
        </el-form-item>
...
import {
  ...
  delGraftingclipap,
  addGraftingclipap,
  updateGraftingclipap,
  exportGraftingclipap,
} from "@/api/consume/graftingclipap";
...
export default {
  ...
  watch: {
    "$route.path": function (newVal, oldVal) {
      if (newVal === "/production/consume/consume7/graftingclipap") {
        this.getList();
      }
    },
  },
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listGraftingclipap(this.queryParams).then((response) => {
        this.graftingclipapList = response.rows;
    ...
    handleApply(row) {
      ...
      getGraftingclipap(graftingclipApplyId).then((response) => {
        ...
        if (valid) {
          if (this.form.graftingclipApplyId != null) {
            ...
            upAmount(this.form)
              ...
              .then((res) => {
                if (code == 200) {
                  upCumStatus(this.form.graftingclipApplyId).then(
                    (response) => {}
                  );
    ...
    handleBack(row) {
      ...
      this.$confirm("是否确认退回嫁接夹领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return back(graftingclipApplyIds);
        })
        .then(() => {
          this.getList();
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.graftingclipApplyId != null) {
            updateGraftingclipap(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGraftingclipap(this.form).then((response) => {
    ...
    handleDelete(row) {
      const graftingclipApplyIds = row.graftingclipApplyId || this.ids;
      this.$confirm("是否确认删除嫁接夹领用?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delGraftingclipap(graftingclipApplyIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.graftingclipapList.length; i++) {
          queryParams.exportIds.push(this.graftingclipapList[i].graftingclipApplyId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有嫁接夹领用数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportGraftingclipap(queryParams);
        })
        .then((response) => {
