<template>
  <div class="app-container">
    <el-form
      ...
      label-width="68px"
    >
      <el-form-item label="类型" prop="regulatorType">
        <el-input
      ...
      <el-form-item label="名称" prop="regulatorName">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['production:regulatorre:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['production:regulatorre:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['production:regulatorre:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['production:regulatorre:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    <el-table
      v-loading="loading"
      :data="regulatorreList"
      @selection-change="handleSelectionChange"
    >
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="编号" align="center" type="index" />
      <el-table-column label="调节剂类型" align="center" prop="regulatorType" />
      <el-table-column label="调节剂名称" align="center" prop="regulatorName" />
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['production:regulatorre:permit']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn(scope.row)"
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="类型" prop="regulatorTypeId">
          ...
          >
            <el-option
              v-for="item in regulatorTypeList"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            ></el-option>
        ...
        <el-form-item label="名称" prop="regulatorNameId">
          ...
          >
            <el-option
              v-for="item in regulatorNameList"
              :key="item.id"
              :label="item.name"
              :value="item.id"
            ></el-option>
...
import {
  ...
  updateRegulatorre,
  exportRegulatorre,
} from "@/api/productionre/regulatorre";
...
export default {
  ...
  methods: {
    ...
    handlethrough(row) {
      ...
      const regulagorReturnId = row.regulagorReturnId || this.ids;
      this.$confirm("是否确认通过该申请信息？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then((response) => {
        console.log("1111", regulagorReturnId);
        getRegulatorre(regulagorReturnId).then((response) => {
          let data = {};
          data = response.data;
          data.approval = 1;
          console.log("data", data);
          updateRegulatorre(data).then((response) => {
    ...
    getList() {
      ...
      listRegulatorre(this.queryParams).then((response) => {
        // if(this.loginUserName!=null){
        //   console.log("111")
        //   this.regulatorreList=[]
        //   for(var i=0;i<response.rows.length;i++){
        //     if(this.loginUserName==response.rows[i].userName){
        //       this.regulatorreList.push(response.rows[i]);
        //     }
        //   }
        //   this.total = response.total;
        //   console.log("11this.regulatorreList",this.regulatorreList)
        // }

        // else {
        this.regulatorreList = response.rows;
        this.total = response.total;
        console.log("22this.regulatorreList", this.regulatorreList);
        // }
        this.loading = false;
      });
    },
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.regulagorReturnId);
    ...
    getTypeList() {
      this.loading = true;
      let list = [];
      getRegulatorType().then((response) => {
        let status = null;
      ...
      this.regulatorTypeList = list;
    },
    getTypeSelected(val) {
      for (let i = 0; i < this.regulatorTypeList.length; i++) {
        if (this.regulatorTypeList[i].id === val) {
          this.form.regulatorTypeId = this.regulatorTypeList[i].id;
        }
      }
    },
    getNameList() {
      this.loading = true;
      let list = [];
      getRegulatorName().then((response) => {
        let status = null;
        for (let i = 0; i < response.rows.length; i++) {
          status = response.rows[i].value;
          list.push({ id: response.rows[i].key, name: status });
      ...
      this.regulatorNameList = list;
    },
    getNameSelected(val) {
      for (let i = 0; i < this.regulatorNameList.length; i++) {
        if (this.regulatorNameList[i].id === val) {
          this.form.regulatorNameId = this.regulatorNameList[i].id;
        }
      }
      this.getCompanyList(this.form.regulatorNameId);
      this.getContentList(this.form.regulatorNameId);
    },
    /** 获取品牌下拉框列表 */
    getBrandList(typeId) {
      var that = this;
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.regulagorReturnId != null) {
            updateRegulatorre(this.form).then((response) => {
            ...
            addRegulatorre(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      let that = this;
      that.form2 = this.form2;
      const regulagorReturnIds = row.regulagorReturnId || this.ids;
      this.$confirm("是否确认删除调节剂退回?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delRegulatorre(regulagorReturnIds).then((response) => {
    ...
    handleExport() {
      ...
      queryParams.regulagorReturnIds = [];
      if (this.ids.length == 0) {
        for (var i = 0; i < this.regulatorreList.length; i++) {
          queryParams.regulagorReturnIds.push(
            this.regulatorreList[i].regulagorReturnId
          );
        }
      ...
      this.$confirm("是否确认导出所有调节剂退回数据项?", "警告", {
        ...
        .then(() => {
          ...
          return exportRegulatorre(queryParams);
        })
        .then((response) => {
