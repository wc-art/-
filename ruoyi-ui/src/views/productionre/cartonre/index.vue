<template>
  <div class="app-container">
    <el-form
      ...
      label-width="68px"
    >
      <el-form-item label="类型" prop="cartonType">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['production:cartonre:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['production:cartonre:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['production:cartonre:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['production:cartonre:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['production:cartonre:permit']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn(scope.row)"
...
import {
  ...
  updateCartonre,
  exportCartonre,
} from "@/api/productionre/cartonre";
...
export default {
  ...
  methods: {
    ...
    handlethrough(row) {
      ...
      const cartonReturnId = row.cartonReturnId || this.ids;
      this.$confirm("是否确认通过该申请信息？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then((response) => {
        console.log("1111", cartonReturnId);
        getCartonre(cartonReturnId).then((response) => {
          let data = {};
          data = response.data;
          data.approval = 1;
          console.log("data", data);
          updateCartonre(data).then((response) => {
    ...
    getList() {
      ...
      listCartonre(this.queryParams).then((response) => {
        // if(this.loginUserName!=null){
        //   console.log("111")
        //   this.cartonreList=[]
        //   for(var i=0;i<response.rows.length;i++){
        //     if(this.loginUserName==response.rows[i].userName){
        //       this.cartonreList.push(response.rows[i]);
        //     }
        //   }
        //   this.total = response.total;
        //   console.log("11this.cartonreList",this.cartonreList)
        // }

        // else {
        this.cartonreList = response.rows;
        this.total = response.total;
        console.log("22this.cartonreList", this.cartonreList);
        // }
        this.loading = false;
      });
    },
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.cartonReturnId);
    ...
    getTypeList() {
      ...
      this.cartonTypeList = list;
    },
    getTypeSelected(val) {
      for (let i = 0; i < this.cartonTypeList.length; i++) {
        if (this.cartonTypeList[i].id === val) {
          this.form.cartonTypeId = this.cartonTypeList[i].id;
        }
      }
      this.getCompanyList(this.form.cartonTypeId);
      this.getSpecificationList(this.form.cartonTypeId);
    },
    /** 获取规格下拉框列表 */
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.cartonReturnId != null) {
            updateCartonre(this.form).then((response) => {
            ...
            addCartonre(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      let that = this;
      that.form2 = this.form2;
      const cartonReturnIds = row.cartonReturnId || this.ids;
      this.$confirm("是否确认删除纸箱退回?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delCartonre(cartonReturnIds).then((response) => {
    ...
    handleExport() {
      ...
      queryParams.cartonReturnIds = [];
      if (this.ids.length == 0) {
        for (var i = 0; i < this.cartonreList.length; i++) {
          queryParams.cartonReturnIds.push(this.cartonreList[i].cartonReturnId);
        }
      ...
      this.$confirm("是否确认导出所有纸箱退回数据项?", "警告", {
        ...
        .then(() => {
          ...
          return exportCartonre(queryParams);
        })
        .then((response) => {
