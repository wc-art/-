<template>
  <div class="app-container">
    <el-form
      ...
      label-width="68px"
    >
      <el-form-item label="类型" prop="greenhousefilmType">
        <el-input
          v-model="queryParams.greenhousefilmType"
          placeholder="请输入类型"
          clearable
          size="small"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['production:greenhousefilmre:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          :disabled="setTopBtn() || single"
          type="success"
          plain
          icon="el-icon-edit"
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['production:greenhousefilmre:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          :disabled="setTopBtn() || multiple"
          type="danger"
          plain
          icon="el-icon-delete"
          size="mini"
          @click="handleDelete"
          v-hasPermi="['production:greenhousefilmre:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['production:greenhousefilmre:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['production:greenhousefilmre:permit']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn(scope.row)"
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="类型" prop="greenhousefilmTypeId">
          ...
          >
            <el-option
              v-for="(item, index) in greenhousefilmTypeList"
              :key="index"
              :label="item.name"
              :value="item.id"
            ></el-option>
        ...
        <el-form-item label="厚度" prop="thickness">
          <el-select v-model="form.thickness" placeholder="请输入厚度">
            <el-option
...
import {
  ...
  delGreenhousefilmre,
  addGreenhousefilmre,
  updateGreenhousefilmre,
  exportGreenhousefilmre,
} from "@/api/productionre/greenhousefilmre";
import { getUserId } from "@/api/system/user";
export default {
  name: "Greenhousefilmre",
  components: {},
  data() {
    ...
    return {
      ...
      queryParams: {
        ...
        greenhousefilmReturnIds: [],
        pageNum: 1,
        pageSize: 10,
        greenhousefilmTypeId: null,
        greenhousefilmType: null,
        thickness: null,
        originCompany: null,
        returnTime: null,
        userId: null,
  ...
  methods: {
    ...
    handlethrough(row) {
      ...
      const greenhousefilmReturnId = row.greenhousefilmReturnId || this.ids;
      this.$confirm("是否确认通过该申请信息？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then((response) => {
        console.log("1111", greenhousefilmReturnId);
        getGreenhousefilmre(greenhousefilmReturnId).then((response) => {
          let data = {};
          data = response.data;
          data.approval = 1;
          console.log("data", data);
          updateGreenhousefilmre(data).then((response) => {
    ...
    getList() {
      ...
      listGreenhousefilmre(this.queryParams).then((response) => {
        // if (this.loginUserName != null) {
        //   console.log("111");
        //   this.greenhousefilmreList = [];
        //   for (var i = 0; i < response.rows.length; i++) {
        //     if (this.loginUserName == response.rows[i].userName) {
        //       this.greenhousefilmreList.push(response.rows[i]);
        //     }
        //   }
        //   this.total = response.total;
        //   console.log("11this.greenhousefilmreList", this.greenhousefilmreList);
        // } else {
        this.greenhousefilmreList = response.rows;
        this.total = response.total;
        console.log("22this.greenhousefilmreList", this.greenhousefilmreList);
        // }
        this.loading = false;
      });
    },
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.greenhousefilmReturnId);
    ...
    getTypeList() {
      ...
      this.greenhousefilmTypeList = list;
    },
    getTypeSelected(val) {
      for (let i = 0; i < this.greenhousefilmTypeList.length; i++) {
        if (this.greenhousefilmTypeList[i].id === val) {
          this.form.greenhousefilmTypeId = this.greenhousefilmTypeList[i].id;
        }
      }
      this.getCompanyList(this.form.greenhousefilmTypeId);
      this.getThicknessList(this.form.greenhousefilmTypeId);
    },
    /** 获取规格下拉框列表 */
    getThicknessList(typeId) {
      var that = this;
      this.loading = true;
      getThickness(typeId).then((response) => {
        that.thicknessList = response.rows;
        this.loading = false;
      });
    },
    /** 获取公司下拉框列表 */
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.greenhousefilmReturnId != null) {
            updateGreenhousefilmre(this.form).then((response) => {
            ...
            addGreenhousefilmre(this.form).then((response) => {
    ...
    handleDelete(row) {
      ...
      let that = this;
      that.form2 = this.form2;
      const greenhousefilmReturnIds = row.greenhousefilmReturnId || this.ids;
      this.$confirm("是否确认删除棚膜退回?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delGreenhousefilmre(greenhousefilmReturnIds).then(
            (response) => {
    ...
    handleExport() {
      ...
      queryParams.greenhousefilmReturnIds = [];
      if (this.ids.length == 0) {
        for (var i = 0; i < this.greenhousefilmreList.length; i++) {
          queryParams.greenhousefilmReturnIds.push(
            this.greenhousefilmreList[i].greenhousefilmReturnId
          );
        }
      } else if (this.ids.length != 0) {
        queryParams.greenhousefilmReturnIds = this.ids;
      }
      this.$confirm("是否确认导出所有棚膜退回数据项?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportGreenhousefilmre(queryParams);
        })
        .then((response) => {
