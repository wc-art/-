      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="品牌" prop="brand">
          <el-select v-model="form.brand" placeholder="请选择品牌">
            <el-option
              v-for="(item, index) in plug_amount"
              :key="index"
              :label="item.brand"
              :value="item.brand"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm("是否确认删除该穴盘申请领用详细?", "提示", {
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.applyList.length; i++) {
          queryParams.exportIds.push(this.applyList[i].plugApplyId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有穴盘申请领用详细数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
