...
export default {
  ...
  created() {
    Promise.all([this.getPlugTypes(), this.getVegertables()]).then((res) => {
      this.getList();
    });
  },
  ...
  methods: {
    ...
    formdata_plug(data) {
      ...
      data.forEach((type) => {
        let type_name = type_map.get(type.plugTypeId);
        type.plugType = type_name;
      });
    },
    //查询穴盘类型
    getPlugTypes() {
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.orderList.length; i++) {
          queryParams.exportIds.push(this.orderList[i].plugPurchaseApplyId);
        }
