...
export default {
  ...
  created() {
    ...
    Promise.all([this.getUserID(), this.getBladeTypes()]).then((res) => {
      this.getList();
    });
  },
  ...
  methods: {
    ...
    getAmount1() {
      var data = {};
      data.bladeTypeId = this.form.bladeTypeId;
      this.form.brand = null;
      this.form.originCompany = null;
      listBladeAmount(data).then((response) => {
        this.blade_amount = response.rows;
      });
    },
    //获取库存
    getAmount2() {
    ...
    handleDelete(row) {
      ...
      this.$confirm("是否确认删除该刀片临时申请领用详细?", "提示", {
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出所有刀片临时申请领用详细数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
