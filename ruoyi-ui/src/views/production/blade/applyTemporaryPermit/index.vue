<template>
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="刀片品牌" prop="brand">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['blade:applyTemporary:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['blade:applyTemporary:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['blade:applyTemporary:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['blade:applyTemporary:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    >
      ...
      <el-table-column label="刀片类型" align="center" prop="bladeType" />
      <el-table-column label="刀片品牌" align="center" prop="brand" />
      <el-table-column label="购自公司" align="center" prop="originCompany" />
      <el-table-column label="申请量" align="center" prop="applyAmount" />
      <el-table-column label="库存量" align="center" prop="bladeTypeId">
        <template slot-scope="scope">
          {{
            (bladeAmountList.filter(
              (item) =>
                item.bladeTypeId == scope.row.bladeTypeId &&
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['blade:applyTemporary:permit1']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['blade:applyTemporary:permit2']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn2(scope.row)"
    ...
    <!--          <el-select v-model="form.bladeTypeId" placeholder="请选择刀片类型">-->
    <!--            <el-option-->
    <!--              v-for="type in blade_type"-->
    <!--              :key="type.bladeTypeId"-->
    <!--              :value="type.bladeTypeId"-->
    ...
    <!--          <el-input v-model="form.brand" placeholder="请输入刀片品牌" />-->
    <!--        </el-form-item>-->
...
} from "@/api/production/blade/applyTemporary";
import { listBladeType } from "@/api/production/blade/bladeType";
import { listBladeAmount } from "@/api/production/blade/bladeAmount";
...
export default {
  ...
  methods: {
    ...
    getBladeAmountList() {
      return new Promise((resolve, rejected) => {
        listBladeAmount(this.queryParams).then((response) => {
          ...
          console.log("bladeAmountList", this.bladeAmountList);
    ...
    handleThroughProduct(row) {
      ...
      const bladeApplyTemporaryId = row.bladeApplyTemporaryId || this.ids;
      let data = {};
      data.bladeApplyTemporaryId = bladeApplyTemporaryId;
      data.approvalStatusProduct = 1;
      this.$confirm("是否确认通过该刀片审核?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return updateApplyTemporary(data).then((response) => {
    ...
    //格式化刀片类型
    formdata_type(data) {
      let blade_type_map = new Map();
      this.blade_type.forEach((type) => {
        blade_type_map.set(type.bladeTypeId, type.bladeType);
      });
      data.forEach((type) => {
        let blade_type_name = blade_type_map.get(type.bladeTypeId);
        type.bladeType = blade_type_name;
      });
    },
    //查询种类
    getBladeTypes() {
      return new Promise((resolve, rejected) => {
        listBladeType().then((response) => {
          this.blade_type = response.rows;
        });
        resolve();
      });
    },
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.bladeApplyTemporaryId);
    ...
    handleUpdate(row) {
      this.reset();
      const bladeApplyTemporaryId = row.bladeApplyTemporaryId || this.ids;
      getApplyTemporary(bladeApplyTemporaryId).then((response) => {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.bladeApplyTemporaryId != null) {
    ...
    handleDelete(row) {
      ...
      let that = this;
      that.form2 = this.form2;
      const bladeApplyTemporaryIds = row.bladeApplyTemporaryId || this.ids;
      this.$confirm("是否确认删除该刀片临时申请?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delApplyTemporary(bladeApplyTemporaryIds).then((response) => {
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.applyTemporaryList.length; i++) {
          queryParams.exportIds.push(this.applyTemporaryList[i].bladeApplyTemporaryId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有刀片临时申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
