        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['blade:apply:permit1']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['blade:apply:permit2']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn2(scope.row)"
...
import {
  ...
  updateApply,
  exportApply,
} from "@/api/production/blade/apply";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        bladeNameId: [
          { required: true, message: "刀片名称不能为空", trigger: "blur" },
        ],
        orderId: [
  ...
  methods: {
    ...
    getBladeAmountList() {
      return new Promise((resolve, rejected) => {
        listBladeAmount(this.queryParams).then((response) => {
          ...
          console.log(this.bladeAmountList);
          resolve();
        });
      });
    },
    ...
    handleThroughProduct(row) {
      ...
      const bladeApplyId = row.bladeApplyId || this.ids;
      let data = {};
      data.bladeApplyId = bladeApplyId;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.bladeApplyId);
    ...
    handleUpdate(row) {
      this.reset();
      const bladeApplyId = row.bladeApplyId || this.ids;
      getApply(bladeApplyId).then((response) => {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.bladeApplyId != null) {
    ...
    handleDelete(row) {
      ...
      let that = this;
      that.form2 = this.form2;
      const bladeApplyIds = row.bladeApplyId || this.ids;
      this.$confirm("是否确认删除该刀片申请?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delApply(bladeApplyIds).then((response) => {
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.applyList.length; i++) {
          queryParams.exportIds.push(this.applyList[i].bladeApplyId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有刀片申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
