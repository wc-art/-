  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="刀片类型" prop="bladeTypeId">
        <el-select v-model="queryParams.bladeTypeId" placeholder="请选择类型" clearable size="small">
          <el-option
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['system:bladeApply:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['system:bladeApply:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table v-loading="loading" :data="bladeApplyList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="预计使用时间" align="center" prop="outgreenhouseTime" width="100">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="刀片类型" prop="bladeTypeId">
          <el-select v-model="form.bladeTypeId" placeholder="请选择类型" clearable size="small">
            <el-option
              v-for="type in blades_type"
              :key="type.bladeTypeId"
              :value="type.bladeTypeId"
...
import { listBladeApply, getBladeApply, delBladeApply, addBladeApply, updateBladeApply, exportBladeApply } from "@/api/production/blade/purchaseApply";
...
orderByColumn:"create_time",
  ...
  created() {
    Promise.all([this.getBlades()]).then((res)=>{
      this.getList();
    })
  ...
  methods: {
    ...
    formdata_blade(data){
      ...
      data.forEach(type => {
        let blade_name = type_map.get(type.bladeTypeId)
        type.bladeType = blade_name
      });
    },
    //查询刀片类型
    getBlades(){
    ...
    getList() {
      this.loading = true;
      listBladeApply(this.queryParams).then(response => {
        this.bladeApplyList = response.rows;
        this.total = response.total;
        this.loading = false;
        this.formdata_blade(this.bladeApplyList)
        console.log(this.bladeApplyList)
      });
    },
    // 取消按钮
    cancel() {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.bladePurchaseApplyId)
    ...
    handleUpdate(row) {
      this.reset();
      const bladePurchaseApplyId = row.bladePurchaseApplyId || this.ids
      getBladeApply(bladePurchaseApplyId).then(response => {
    ...
    handleDelete(row) {
      ...
      const bladePurchaseApplyIds = row.bladePurchaseApplyId || this.ids;
      this.$confirm('是否确认删除该刀片购买申请的数据项?', "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
        return delBladeApply(bladePurchaseApplyIds).then(response =>{
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有刀片购买申请数据项?', "提示", {
        ...
        return exportBladeApply(queryParams);
      }).then(response => {
