      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['system:bladeApply:permit']"
            >通过</el-button
          >
          <el-button
            v-bind:disabled="setpassBtn1(scope.row)"
...
export default {
  ...
  methods: {
    ...
    handlethrough(row) {
      ...
      const bladePurchaseApplyId = row.bladePurchaseApplyId || this.ids;
      let data = {};
      data.bladePurchaseApplyId = bladePurchaseApplyId;
      ...
      this.$confirm("是否确认通过该条审核?", "提示", {
        ...
        .then(function () {
          return updateBladeApply(data).then((response) => {
    ...
    handleNotThrogh(row) {
      ...
      const bladePurchaseApplyIds = row.bladePurchaseApplyId || this.ids;
      let data = {};
      data.bladePurchaseApplyId = bladePurchaseApplyIds;
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.orderList.length; i++) {
          queryParams.exportIds.push(this.bladeApplyList[i].bladePurchaseApplyId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出刀片购买申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
