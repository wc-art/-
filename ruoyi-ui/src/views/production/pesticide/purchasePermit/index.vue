<template>
  <div class="app-container">
    ...
    <el-table
      v-loading="loading"
      :data="pesticidePurchaseApplyList"
      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['system:pesticideApply:permit']"
            >通过</el-button
          >
          <el-button
            v-bind:disabled="setpassBtn1(scope.row)"
...
import {
  listPesticidePurchaseApply,
  getPesticidePurchaseApply,
  delPesticidePurchaseApply,
  addPesticidePurchaseApply,
  updatePesticidePurchaseApply,
  exportPesticidePurchaseApply,
...
export default {
  ...
  created() {
    Promise.all([this.getPesticideTypes(), this.getPesticideNames()]).then(
      (res) => {
        this.getList();
      }
    );
  ...
  methods: {
    ...
    handlethrough(row) {
      ...
      const pesticidePurchaseApplyId = row.pesticidePurchaseApplyId || this.ids;
      let data = {};
      data.pesticidePurchaseApplyId = pesticidePurchaseApplyId;
      ...
      this.$confirm("是否确认通过该条审核?", "提示", {
        ...
        .then(function () {
          return updatePesticidePurchaseApply(data).then((response) => {
    ...
    handleDelete(row) {
      ...
      const pesticidePurchaseApplyIds =
        row.pesticidePurchaseApplyId || this.ids;
      this.$confirm("是否确认删除该农药购买申请?", "提示", {
        ...
        .then(function () {
          return delPesticidePurchaseApply(pesticidePurchaseApplyIds).then(
            (response) => {
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.pesticidePurchaseApplyList.length; i++) {
          queryParams.exportIds.push(this.pesticidePurchaseApplyList[i].pesticidePurchaseApplyId);
        }
