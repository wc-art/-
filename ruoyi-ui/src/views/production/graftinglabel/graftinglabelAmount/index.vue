    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['production:graftinglabelAmount:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['production:graftinglabelAmount:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['production:graftinglabelAmount:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['production:graftinglabelAmount:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
...
import {
  ...
  listGraftinglabelAmount,
  getGraftinglabelAmount,
  delGraftinglabelAmount,
  addGraftinglabelAmount,
  updateGraftinglabelAmount,
  exportGraftinglabelAmount,
} from "@/api/production/graftinglabel/graftinglabelAmount";

export default {
  name: "GraftinglabelAmount",
  components: {},
  data() {
    return {
      // 遮罩层
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listGraftinglabelAmount(this.queryParams).then((response) => {
        this.graftinglabelAmountList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.graftinglabelAmountId);
    ...
    handleUpdate(row) {
      this.reset();
      const graftinglabelAmountId = row.graftinglabelAmountId || this.ids;
      getGraftinglabelAmount(graftinglabelAmountId).then((response) => {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.graftinglabelAmountId != null) {
            updateGraftinglabelAmount(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addGraftinglabelAmount(this.form).then((response) => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const graftinglabelAmountIds = row.graftinglabelAmountId || this.ids;
      this.$confirm("是否确认删除该嫁接签量?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delGraftinglabelAmount(graftinglabelAmountIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.graftinglabelAmountList.length; i++) {
          queryParams.exportIds.push(this.graftinglabelAmountList[i].graftinglabelAmountId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有嫁接签量数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportGraftinglabelAmount(queryParams);
        })
        .then((response) => {
