...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        graftinglabelTypeId: [
          { required: true, message: "嫁接签类型不能为空", trigger: "change" },
        ],
        applyAmount: [
  ...
  created() {
    Promise.all([this.getVegertables(), this.getGraftinglabels()]).then(
      (res) => {
        this.getList();
      }
    );
  ...
  methods: {
    ...
    formdata_graftinglabel(data) {
      ...
      data.forEach((type) => {
        let type_name = type_map.get(type.graftinglabelTypeId);
        type.graftinglabelType = type_name;
      });
    },
    //查询嫁接签类型
    getGraftinglabels() {
    ...
    getList() {
      ...
      listOrder(this.queryParams).then((response) => {
        ...
        this.loading = false;
        this.formdata_graftinglabel(this.orderList);
      });
    },
        ...
        if (valid) {
          if (this.form.graftinglabelPurchaseApplyId != null) {
            ...
            addGraftinglabelApply(this.form).then((response) => {
              ...
              this.open = false;
              this.getList();
              this.$router.push("/production/purchaseApply/graftinglabel");
            });
          }
        }
      });
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.orderList.length; i++) {
          queryParams.orderIds.push(this.orderList[i].graftinglabelPurchaseApplyId);
        }
      } else if (this.ids.length != 0) {
        queryParams.orderIds = this.ids;
      }
      this.$confirm("是否确认导出嫁接签购买申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
