<template>
  <div class="app-container">
    <el-form
      ...
      label-width="68px"
    >
      <el-form-item label="类型" prop="graftinglabelTypeId" label-width="85px">
        <el-select
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['graftinglabel:applyTemporary:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['graftinglabel:applyTemporary:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['graftinglabel:applyTemporary:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['graftinglabel:applyTemporary:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    >
      ...
      <el-table-column label="库存量" align="center" prop="graftingclipTypeId">
        <template slot-scope="scope">
          <span>{{
            (graftingclipAmountList.filter(
              (item) =>
                item.graftingclipTypeId == scope.row.graftingclipTypeId &&
                item.graftinglabelSpecification ==
                  scope.row.graftinglabelSpecification &&
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['graftinglabel:applyTemporary:permit1']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['graftinglabel:applyTemporary:permit2']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn2(scope.row)"
    ...
    <!--          <el-select v-model="form.graftinglabelTypeId" placeholder="请选择嫁接签类型">-->
    <!--            <el-option-->
    <!--              v-for="type in graftinglabel_type"-->
    <!--              :key="type.graftinglabelTypeId"-->
    <!--              :value="type.graftinglabelTypeId"-->
    ...
    <!--        </el-form-item>-->
    <!--        <el-form-item label="规格" prop="graftinglabelSpecification">-->
    <!--          <el-input v-model="form.graftinglabelSpecification" placeholder="请输入规格" />-->
    <!--        </el-form-item>-->
    <!--        <el-form-item label="购自公司" prop="originCompany">-->
...
} from "@/api/production/graftinglabel/applyTemporary";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        graftinglabelNameId: [
          { required: true, message: "嫁接签名称不能为空", trigger: "blur" },
        ],
        applyAmount: [
  ...
  created() {
    Promise.all([
      this.getUserID(),
      this.getGraftinglabelTypes(),
      this.getGraftingclipAmountList(),
    ]).then((res) => {
      this.getList();
    });
  },
  ...
  methods: {
    ...
    handleThroughProduct(row) {
      ...
      let data = {};
      data.graftinglabelApplyTemporaryId = graftinglabelApplyTemporaryId;
      data.approvalStatusProduct = 1;
      this.$confirm("是否确认通过该嫁接签审核?", "提示", {
    ...
    handleUpdate(row) {
      ...
      const graftinglabelApplyTemporaryId =
        row.graftinglabelApplyTemporaryId || this.ids;
      getApplyTemporary(graftinglabelApplyTemporaryId).then((response) => {
        this.form = response.data;
        this.open = true;
        this.title = "修改嫁接签临时申请";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      const graftinglabelApplyTemporaryIds =
        row.graftinglabelApplyTemporaryId || this.ids;
      this.$confirm("是否确认删除该嫁接签临时申请?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delApplyTemporary(graftinglabelApplyTemporaryIds).then(
            (response) => {
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.applyTemporaryList.length; i++) {
          queryParams.exportIds.push(this.applyTemporaryList[i].graftinglabelApplyTemporaryId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有嫁接签临时申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
