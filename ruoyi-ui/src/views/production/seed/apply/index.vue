...
export default {
  ...
  data() {
    return {
      ...
      // 表单校验
      rules: {
        seedType: [{ required: true, message: "种子类型不能为空" }],
        cropTypeId: [
  ...
  created() {
    ...
    Promise.all([this.getUserId(), this.getVegertables()]).then((res) => {
      this.getList();
    });
  ...
  methods: {
    ...
    //获取库存
    getAmount() {
      listSeedAmount(this.form).then((response) => {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          addApply(this.form).then((response) => {
            ...
            this.open = false;
            this.getList();
            this.$router.push("/production/ApplyUseDetail/seed");
          });
        }
      });
    },
