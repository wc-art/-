  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="种子类型" prop="seedType">
          <el-select
            v-model="form.seedType"
            placeholder="请选择种子类型:接穗| 砧木"
          >
            <el-option
              v-for="type in seed_type"
              :key="type.dictValue"
...
import {
  ...
  updateApply,
  exportApply,
} from "@/api/production/seed/permit";
...
export default {
  ...
  created() {
    Promise.all([this.getVegertables()]).then((res) => {
      this.getList();
    });
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      const seedPurchaseApplyId = row.seedPurchaseApplyId || this.ids;
      this.$confirm("是否确认删除该种子审核信息?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delApply(seedPurchaseApplyId).then((response) => {
    ...
    handlethrough(row) {
      ...
      const seedPurchaseApplyId = row.seedPurchaseApplyId || this.ids;
      let data = {};
      data.seedPurchaseApplyId = seedPurchaseApplyId;
    ...
    handleNotThrogh(row) {
      ...
      let data = {};
      data.seedPurchaseApplyId = seedPurchaseApplyIds;
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.applyList.length; i++) {
          queryParams.exportIds.push(this.applyList[i].seedPurchaseApplyId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有种子审核数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
