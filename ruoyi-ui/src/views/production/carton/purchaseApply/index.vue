...
export default {
  ...
  created() {
    Promise.all([this.getVegertables(), this.getCartons()]).then((res) => {
      this.getList();
    });
  },
  ...
  methods: {
    ...
    formdata_carton(data) {
      ...
      data.forEach((type) => {
        let type_name = type_map.get(type.cartonTypeId);
        type.cartonType = type_name;
      });
    },
    //查询纸箱类型
    getCartons() {
    ...
    getList() {
      ...
      listOrder(this.queryParams).then((response) => {
        ...
        this.loading = false;
        this.formdata_carton(this.orderList);
      });
    },
        ...
        if (valid) {
          if (this.form.cartonPurchaseApplyId != null) {
            ...
            addCartonApply(this.form).then((response) => {
              ...
              this.open = false;
              this.getList();
              this.$router.push("/production/purchaseApply/carton");
            });
          }
        }
      });
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.orderList.length; i++) {
          queryParams.orderIds.push(this.orderList[i].cartonPurchaseApplyId);
        }
