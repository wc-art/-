      ...
      >
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['system:cartonApply:permit']"
            >通过</el-button
          >
          <el-button
            v-bind:disabled="setpassBtn1(scope.row)"
...
import {
  ...
  updateCartonApply,
  exportCartonApply,
} from "@/api/production/carton/purchaseApply";
...
export default {
  ...
  created() {
    Promise.all([this.getCartons()]).then((res) => {
      this.getList();
    });
  },
  methods: {
    ...
    handlethrough(row) {
      ...
      const cartonPurchaseApplyId = row.cartonPurchaseApplyId || this.ids;
      let data = {};
      data.cartonPurchaseApplyId = cartonPurchaseApplyId;
      ...
      this.$confirm("是否确认通过该条审核?", "提示", {
        ...
        .then(function () {
          return updateCartonApply(data).then((response) => {
    ...
    handleNotThrogh(row) {
      ...
      const cartonPurchaseApplyIds = row.cartonPurchaseApplyId || this.ids;
      let data = {};
      data.cartonPurchaseApplyId = cartonPurchaseApplyIds;
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.cartonApplyList.length; i++) {
          queryParams.exportIds.push(this.cartonApplyList[i].cartonPurchaseApplyId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出纸箱购买申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
