...
export default {
    ...
    return {
      ...
      rules: {
        greenhousefilmTypeId: [
          { required: true, message: "棚膜类型不能为空", trigger: "blur" },
        ],
        seedType: [
          {
            required: true,
  ...
  created() {
    Promise.all([this.getVegertables(), this.getGreenhousefilmTypes()]).then(
      (res) => {
        this.getList();
      }
    );
        ...
        if (valid) {
          if (this.form.greenhousefilmPurchaseApplyId != null) {
            ...
            addGreenhousefilmpply(this.form).then((response) => {
              ...
              this.getList();
              this.$router.push(
                "/production/purchaseApply/greenhousefilmApplyDetail"
              );
            });
          }
        }
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.orderList.length; i++) {
          queryParams.exportIds.push(this.orderList[i].greenhousefilmPurchaseApplyId);
        }
