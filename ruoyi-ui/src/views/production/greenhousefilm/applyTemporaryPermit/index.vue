<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['greenhousefilm:applyTemporary:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['greenhousefilm:applyTemporary:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['greenhousefilm:applyTemporary:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['greenhousefilm:applyTemporary:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    >
      ...
      <el-table-column
        label="库存量"
        align="center"
        prop="greenhousefilmTypeId"
      >
        <template slot-scope="scope">
          <span>{{
            (greenhousefilmAmountList.filter(
              (item) =>
                item.greenhousefilmTypeId == scope.row.greenhousefilmTypeId &&
                item.thickness == scope.row.thickness &&
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['greenhousefilm:applyTemporary:permit1']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['greenhousefilm:applyTemporary:permit2']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn2(scope.row)"
    ...
    <!--          <el-select v-model="form.greenhousefilmTypeId" placeholder="请选择棚膜类型" clearable size="small">-->
    <!--            <el-option-->
    <!--              v-for="type in greenhousefilm_type"-->
    <!--              :key="type.greenhousefilmTypeId"-->
    <!--              :value="type.greenhousefilmTypeId"-->
...
} from "@/api/production/greenhousefilm/applyTemporary";
...
export default {
  ...
  created() {
    Promise.all([
      this.getUserID(),
      this.getGreenhousefilmTypes(),
      this.getGreenhousefilmAmountList(),
    ]).then((res) => {
      this.getList();
    });
  },
  methods: {
    //查询棚膜库存量
    getGreenhousefilmAmountList() {
      return new Promise((resolve, rejected) => {
        listGreenhousefilmAmount(this.queryParams).then((response) => {
          this.greenhousefilmAmountList = response.rows;
          console.log(this.greenhousefilmAmountList);
          resolve();
        });
      });
    },
    ...
    handleThroughProduct(row) {
      ...
      let data = {};
      data.greenhousefilmApplyTemporaryId = greenhousefilmApplyTemporaryId;
      data.approvalStatusProduct = 1;
      this.$confirm("是否确认通过该棚膜审核?", "提示", {
    ...
    handleUpdate(row) {
      ...
      const greenhousefilmApplyTemporaryId =
        row.greenhousefilmApplyTemporaryId || this.ids;
      getApplyTemporary(greenhousefilmApplyTemporaryId).then((response) => {
    ...
    handleDelete(row) {
      ...
      const greenhousefilmApplyTemporaryIds =
        row.greenhousefilmApplyTemporaryId || this.ids;
      this.$confirm("是否确认删除该棚膜临时申请?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delApplyTemporary(greenhousefilmApplyTemporaryIds).then(
            (response) => {
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.applyTemporaryList.length; i++) {
          queryParams.exportIds.push(this.applyTemporaryList[i].greenhousefilmApplyTemporaryId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有棚膜临时申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
