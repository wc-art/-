      ...
      >
        <template slot-scope="">
          <el-button
            ...
            size="small"
            @click="handleAdd"
            v-hasPermi="['system:graftingclippurchaseApply:add']"
            width="100"
            >申请</el-button
          >
        </template>
...
export default {
  ...
  created() {
    Promise.all([this.getVegertables(), this.getGraftingclips()]).then(
      (res) => {
        this.getList();
      }
    );
  ...
  methods: {
    ...
    formdata_graftingclip(data) {
      ...
      data.forEach((type) => {
        let type_name = type_map.get(type.graftingclipTypeId);
        type.graftingclipType = type_name;
      });
    },
    //查询嫁接夹类型
    getGraftingclips() {
    ...
    getList() {
      ...
      listOrder(this.queryParams).then((response) => {
        ...
        this.loading = false;
        this.formdata_graftingclip(this.orderList);
      });
    },
        ...
        if (valid) {
          if (this.form.graftingclipPurchaseApplyId != null) {
            ...
            addGraftingclipApply(this.form).then((response) => {
              ...
              this.open = false;
              this.getList();
              this.$router.push("/production/purchaseApply/graftingclip");
            });
          }
        }
      });
    ...
    handleExport() {
      ...
      this.$confirm("是否确认导出嫁接夹购买申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
