      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="产地" prop="originPlace">
          <el-select v-model="form.originPlace" placeholder="请选择产地">
            <el-option label="国产" value="国产" @click.native="getAmount" />
            <el-option label="进口" value="进口" @click.native="getAmount" />
          </el-select>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        graftingclipTypeId: [
          { required: true, message: "嫁接夹类型不能为空", trigger: "change" },
        ],
        orderId: [
  ...
  methods: {
    ...
    //获取库存
    getAmount() {
      listGraftingclipAmount(this.form).then((response) => {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          addApply(this.form).then((response) => {
            ...
            this.open = false;
            this.getList();
            this.$router.push("/production/ApplyUseDetail/graftingclip");
          });
        }
      });
    },
