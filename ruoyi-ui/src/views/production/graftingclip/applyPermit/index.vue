<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['graftingclip:apply:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['graftingclip:apply:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['graftingclip:apply:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['graftingclip:apply:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
          ...
          <span>{{
            (graftingclipAmountList.filter(
              (item) =>
                ...
                item.graftingclipSpecification ==
                  scope.row.graftingclipSpecification &&
                item.originCompany == scope.row.originCompany &&
                item.graftingclipSpecificationId ==
                  scope.row.graftingclipSpecificationId
            )[0] || {})["storageAmount"] || 0
          }}</span>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['graftingclip:apply:permit1']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['graftingclip:apply:permit2']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn2(scope.row)"
    ...
    <!--        </el-form-item>-->
    <!--        <el-form-item label="类型" prop="graftingclipTypeId">-->
    <!--          <el-select v-model="form.graftingclipTypeId" placeholder="请选择嫁接夹类型">-->
    <!--            <el-option-->
    <!--              v-for="type in graftingclip_type"-->
    <!--              :key="type.graftingclipTypeId"-->
    <!--              :value="type.graftingclipTypeId"-->
    ...
    <!--          <el-input v-model="form.graftingclipSpecification" placeholder="请输入口径型号" />-->
    <!--        </el-form-item>-->
...
import {
  ...
  updateApply,
  exportApply,
} from "@/api/production/graftingclip/apply";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        graftingclipNameId: [
          { required: true, message: "嫁接夹名称不能为空", trigger: "blur" },
        ],
        orderId: [
  ...
  methods: {
    ...
    handleThroughProduct(row) {
      ...
      const graftingclipApplyId = row.graftingclipApplyId || this.ids;
      let data = {};
      data.graftingclipApplyId = graftingclipApplyId;
      data.approvalStatusProduct = 1;
      this.$confirm("是否确认通过该嫁接夹审核?", "提示", {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.graftingclipApplyId);
    ...
    handleUpdate(row) {
      this.reset();
      const graftingclipApplyId = row.graftingclipApplyId || this.ids;
      getApply(graftingclipApplyId).then((response) => {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.graftingclipApplyId != null) {
    ...
    handleDelete(row) {
      ...
      let that = this;
      that.form2 = this.form2;
      const graftingclipApplyIds = row.graftingclipApplyId || this.ids;
      this.$confirm("是否确认删除该嫁接夹申请?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delApply(graftingclipApplyIds).then((response) => {
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.applyList.length; i++) {
          queryParams.exportIds.push(this.applyList[i].graftingclipApplyId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有嫁接夹申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
