<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['stroma:applyTemporary:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['stroma:applyTemporary:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['stroma:applyTemporary:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['stroma:applyTemporary:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    >
      ...
      <el-table-column label="库存量" align="center" prop="stromaTypeId">
        <template slot-scope="scope">
          <span>{{
            (stromaAmountList.filter(
              (item) =>
                item.stromaTypeId == scope.row.stromaTypeId &&
                item.stromaKindId == scope.row.stromaKindId &&
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['stroma:applyTemporary:permit1']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['stroma:applyTemporary:permit2']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn2(scope.row)"
...
} from "@/api/production/stroma/applyTemporary";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        stromaKindId: [
          { required: true, message: "基质种类外码不能为空", trigger: "blur" },
        ],
        stromaTypeId: [
  ...
  methods: {
    ...
    getstromaAmountList() {
      return new Promise((resolve, rejected) => {
        listStromaAmount(this.queryParams).then((response) => {
          ...
          console.log(this.stromaAmountList);
          resolve();
        });
      });
    },
    ...
    handleThroughProduct(row) {
      ...
      let data = {};
      data.stromaApplyTemporayId = stromaApplyTemporayId;
      data.approvalStatusProduct = 1;
      this.$confirm("是否确认通过该基质审核?", "提示", {
    ...
    getList() {
      ...
      listApplyTemporary(this.queryParams).then((response) => {
        ...
        this.loading = false;
        this.formdata_stroma(this.applyTemporaryList);
        this.formdata_stroma_type(this.applyTemporaryList);
    ...
    handleUpdate(row) {
      this.reset();
      const stromaApplyTemporayId = row.stromaApplyTemporayId || this.ids;
      getApplyTemporary(stromaApplyTemporayId).then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$confirm("是否确认删除该基质临时申请?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delApplyTemporary(stromaApplyTemporayIds).then((response) => {
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.applyTemporaryList.length; i++) {
          queryParams.exportIds.push(this.applyTemporaryList[i].stromaApplyTemporayId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有基质临时申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
