<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['production:fertilizerPurchase:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['production:fertilizerPurchase:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['production:fertilizerPurchase:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['production:fertilizerPurchase:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    <el-table
      v-loading="loading"
      :data="fertilizerPurchaseList"
      @selection-change="handleSelectionChange"
    >
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="编号" align="center" type="index" />
      <el-table-column label="肥料类型" align="center" prop="fertilizerTypeId">
        <template slot-scope="scope">
          <span>{{
            FertilizerTypeList.filter(
              (item) => item.fertilizerTypeId == scope.row.fertilizerTypeId
            )[0].fertilizerType
          }}</span>
        </template>
      </el-table-column>
      <el-table-column label="肥料名称" align="center" prop="fertilizerNameId">
        <template slot-scope="scope">
          <span>{{
            FertilizerNameList.filter(
              (item) => item.fertilizerNameId == scope.row.fertilizerNameId
            )[0].fertilizerName
          }}</span>
        </template>
      </el-table-column>
...
import {
  listFertilizerType,
  listFertilizerName,
  listFertilizerPurchase,
  getFertilizerPurchase,
  delFertilizerPurchase,
  addFertilizerPurchase,
  updateFertilizerPurchase,
  exportFertilizerPurchase,
} from "@/api/production/fertilizer/fertilizerPurchase";
import Editor from "@/components/Editor";

export default {
  name: "FertilizerPurchase",
  components: {
    Editor,
  },
  data() {
    return {
      ...
      rules: {
        ...
        npkContent: [
          { required: true, message: "NPK含量不能为空", trigger: "blur" },
        ],
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listFertilizerPurchase(this.queryParams).then((response) => {
        this.fertilizerPurchaseList = response.rows;
        this.total = response.total;
        this.loading = false;
      });
    },
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map((item) => item.fertilizerPurchaseId);
    ...
    handleUpdate(row) {
      this.reset();
      const fertilizerPurchaseId = row.fertilizerPurchaseId || this.ids;
      getFertilizerPurchase(fertilizerPurchaseId).then((response) => {
    ...
    submitForm() {
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.fertilizerPurchaseId != null) {
            updateFertilizerPurchase(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addFertilizerPurchase(this.form).then((response) => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const fertilizerPurchaseIds = row.fertilizerPurchaseId || this.ids;
      this.$confirm("是否确认删除该化肥购买信息?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delFertilizerPurchase(fertilizerPurchaseIds);
        })
        .then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.fertilizerPurchaseList.length; i++) {
          queryParams.exportIds.push(this.fertilizerPurchaseList[i].fertilizerPurchaseId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有化肥购买信息数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          ...
          return exportFertilizerPurchase(queryParams);
        })
        .then((response) => {
