    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['fertilizer:applyTemporary:add']"
          >申请</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleUpdate"
          v-hasPermi="['fertilizer:applyTemporary:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['fertilizer:applyTemporary:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['fertilizer:applyTemporary:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
...
} from "@/api/production/fertilizer/applyTemporary";
...
export default {
  ...
  methods: {
    ...
    getAmount1() {
      var data = {};
      data.fertilizerTypeId = this.form.fertilizerTypeId;
      data.fertilizerNameId = this.form.fertilizerNameId;
      this.form.npkContent = null;
      this.form.brand = null;
      this.form.originCompany = null;
      listFertilizerAmount(data).then((response) => {
    ...
    getAmount2() {
      ...
      data.fertilizerTypeId = this.form.fertilizerTypeId;
      data.fertilizerNameId = this.form.fertilizerNameId;
      data.npkContent = this.form.npkContent;
      this.form.brand = null;
      this.form.originCompany = null;
      listFertilizerAmount(data).then((response) => {
        this.fertilizer_amount = response.rows;
    ...
    handleUpdate(row) {
      ...
      getApplyTemporary(fertilizerApplyTemporaryId).then((response) => {
    ...
    handleDelete(row) {
      ...
      const fertilizerApplyTemporaryIds =
        row.fertilizerApplyTemporaryId || this.ids;
      this.$confirm("是否确认删除该化肥临时申请?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delApplyTemporary(fertilizerApplyTemporaryIds).then(
            (response) => {
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.applyTemporaryList.length; i++) {
          queryParams.exportIds.push(this.applyTemporaryList[i].fertilizerApplyTemporaryId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有化肥临时申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
