  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="肥料类型" prop="fertilizerTypeId">
        <el-select v-model="queryParams.fertilizerTypeId" placeholder="请选择化肥类型" clearable size="small">
          <el-option
      ...
      <el-form-item label="肥料名称" prop="fertilizerNameId">
        <el-select v-model="queryParams.fertilizerNameId" placeholder="请选择化肥名称" clearable size="small">
          <el-option
    ...
    <el-table v-loading="loading" :data="fertilizerApplyList" @selection-change="handleSelectionChange">
...
import { listFertilizerApply, getFertilizerApply, delFertilizerApply, addFertilizerApply, updateFertilizerApply, exportFertilizerApply } from "@/api/production/fertilizer/purchaseApply";
...
orderByColumn:"create_time",
  ...
  methods: {
    ...
    formdata_type(data) {
      ...
      data.forEach((type) => {
        let fertilizer_type_name = fertilizer_type_map.get(type.fertilizerTypeId);
    ...
    formdata_name(data) {
      ...
      data.forEach((type) => {
        let fertilizer_name_name = fertilizer_name_map.get(type.fertilizerNameId);
    ...
    handleUpdate(row) {
      this.reset();
      const fertilizerPurchaseApplyId = row.fertilizerPurchaseApplyId || this.ids
    ...
    handleDelete(row) {
      ...
      const fertilizerPurchaseApplyIds = row.fertilizerPurchaseApplyId || this.ids;
      this.$confirm('是否确认删除该化肥购买申请?', "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
        return delFertilizerApply(fertilizerPurchaseApplyIds).then(response =>{
