<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <!--          size="mini"-->
      <!--          @click="handleAdd"-->
      <!--          v-hasPermi="['regulator:apply:add']"-->
      <!--        >新增</el-button>-->
      <!--      </el-col>-->
      <!--      <el-col :span="1.5">-->
      <!--        <el-button-->
      ...
      <!--          :disabled="single"-->
      <!--          @click="handleUpdate"-->
      <!--          v-hasPermi="['regulator:apply:edit']"-->
      <!--        >修改</el-button>-->
      <!--      </el-col>-->
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleDelete"
          v-hasPermi="['regulator:apply:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['regulator:apply:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
    ...
    >
      ...
      <el-table-column label="库存量" align="center" prop="regulatorTypeId">
        <template slot-scope="scope">
          <span>{{
            (regulatorAmountList.filter(
              (item) =>
                item.regulatorTypeId == scope.row.regulatorTypeId &&
                item.regulatorNameId == scope.row.regulatorNameId &&
                item.content == scope.row.content &&
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['regulator:apply:permit1']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn1(scope.row)"
        ...
        <template slot-scope="scope">
          ...
          <div style="display: flex">
            <el-button
              ...
              v-hasPermi="['regulator:apply:permit2']"
              >通过</el-button
            >
            <el-button
              v-bind:disabled="setpassBtn2(scope.row)"
    ...
    <!--        </el-form-item>-->
    <!--        <el-form-item label="类型" prop="regulatorTypeId">-->
    <!--          <el-select v-model="form.regulatorTypeId" placeholder="请选择调节剂类型">-->
    <!--            <el-option-->
    <!--              v-for="type in regulator_type"-->
    <!--              :key="type.regulatorTypeId"-->
    <!--              :value="type.regulatorTypeId"-->
    ...
    <!--        </el-form-item>-->
    <!--        <el-form-item label="名称" prop="regulatorNameId">-->
    <!--          <el-select v-model="form.regulatorNameId" placeholder="请选择调节剂名称">-->
    <!--            <el-option-->
    <!--              v-for="name in regulator_name"-->
    <!--              :key="name.regulatorNameId"-->
    <!--              :value="name.regulatorNameId"-->
...
import {
  ...
  updateApply,
  exportApply,
} from "@/api/production/regulator/apply";
...
export default {
  ...
  methods: {
    ...
    handleThroughProduct(row) {
      ...
      let data = {};
      data.regulatorApplyId = regulatorApplyId;
      data.approvalStatusProduct = 1;
      this.$confirm("是否确认通过该调节剂审核?", "提示", {
    ...
    handleUpdate(row) {
      this.reset();
      const regulatorApplyId = row.regulatorApplyId || this.ids;
      getApply(regulatorApplyId).then((response) => {
    ...
    handleDelete(row) {
      ...
      this.$confirm("是否确认删除该调节剂申请?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(function () {
          return delApply(regulatorApplyIds).then((response) => {
    ...
    handleExport() {
      ...
      if (this.ids.length == 0) {
        for (let i = 0; i < this.applyList.length; i++) {
          queryParams.exportIds.push(this.applyList[i].regulatorApplyId);
        }
      } else if (this.ids.length != 0) {
        queryParams.exportIds = this.ids;
      }
      this.$confirm("是否确认导出所有调节剂申请数据项?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
