<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="调节剂类型" prop="regulatorTypeId" label-width="85px">
        <el-select v-model="queryParams.regulatorTypeId" placeholder="请选择调节剂类型" clearable size="small">
          <el-option v-for="type in regulator_type"
      ...
      <el-form-item label="调节剂名称" prop="regulatorNameId" label-width="85px">
        <el-select v-model="queryParams.regulatorNameId" placeholder="请选择调节剂名称" clearable size="small">
          <el-option v-for="name in regulator_name"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:regulatorApply:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:regulatorApply:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table v-loading="loading" :data="regulatorApplyList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="编号" align="center" type="index" />
      <el-table-column label="调节剂类型" align="center" prop="regulatorType" label-width="90px"/>
      <el-table-column label="调节剂名称" align="center" prop="regulatorName" label-width="90px"/>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="调节剂类型" prop="regulatorTypeId">
          <el-select v-model="form.regulatorTypeId" placeholder="请选择调节剂类型" clearable size="small">
            <el-option v-for="type in regulator_type"
                       :key="type.regulatorTypeId"
                       :value="type.regulatorTypeId"
                       :label="type.regulatorType"
        ...
        <el-form-item label="调节剂名称" prop="regulatorNameId">
          <el-select v-model="form.regulatorNameId" placeholder="请选择调节剂名称" clearable>
            <el-option v-for="name in regulator_name"
                       :key="name.regulatorNameId"
                       :value="name.regulatorNameId"
                       :label="name.regulatorName"
...
import { listRegulatorApply, getRegulatorApply, delRegulatorApply, addRegulatorApply, updateRegulatorApply, exportRegulatorApply } from "@/api/production/regulator/purchaseApply";
...
orderByColumn:"create_time",
  ...
  created() {
    Promise.all([
      this.getRegulatorTypes(), this.getRegulatorNames()
    ]).then((res)=>{
      this.getList();
    })
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      const regulatorPurchaseApplyIds = row.regulatorPurchaseApplyId || this.ids;
      this.$confirm('是否确认删除该调节剂购买申请?', "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delRegulatorApply(regulatorPurchaseApplyIds)(response =>{
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有调节剂购买申请数据项?', "提示", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
